...
def _execution_code_commit() -> str:
    ...
    if result.returncode != 0 or len(commit) != 40 or any(character not in '0123456789abcdef' for character in commit):
        raise RuntimeError('EXECUTION_CODE_COMMIT_UNAVAILABLE')
    return commit
...
def synthetic_classification_fixture(contract: FrozenContract, output_root: Path, run_id: str, *, fail_fold_once: int | None=2) -> dict[str,Any]:
    ...
    data=ClassificationCrossfitData(X,y,meta); store=AtomicArtifactStore(ArtifactPolicy.fixture(root,worktree_root=ROOT,formal_output_root=contract.output_root),run_id=run_id,signature_bundle=contract.signatures); state_store=AtomicExecutionStateStore(root); execution_code_commit=_execution_code_commit()
    failed=False; before_fit=0
    try: run_classification_crossfit(data,contract,store,state_store=state_store,failure_injection=fail_fold_once,execution_code_commit=execution_code_commit,trainer_sha256=sha256_file(Path(__file__)))
    ...
    result=run_classification_crossfit(data,contract,store,state_store=state_store,resume=True,execution_code_commit=execution_code_commit,trainer_sha256=sha256_file(Path(__file__)))
