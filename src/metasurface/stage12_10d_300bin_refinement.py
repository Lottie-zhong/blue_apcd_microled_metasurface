...
from metasurface.stage12_10a_240bin_refinement import (
    REJECT_FIELDS, RESULT_FIELDS, bool_from, clearance_ok, estimated_leakage,
    flt, fmt, load_stage11_runner, nearest_bin, read_csv, result_to_metric,
    sizes, wrap180, write_csv,
)
from metasurface.stage12_10b_240bin_broad_family import parse_family

OUTPUT_DIR_NAME = "stage12_10d_h500_lp_300bin_single_dimer_refinement"
MAX_NEW_CANDIDATES = 12
FROZEN_300 = {
    "candidate_id": "H500DIMER2D_006_B240_x_pair_swap_G80_O-30",
    "ratio": 13.460024,
    "Tx": 0.987489,
    "phase_error_deg": 7.474235,
    "matrix_error": 0.272570,
    "estimated_leakage": 0.0734,
}
...
def phase_error_to_300(phase_deg: float) -> float:
    return abs(wrap180(phase_deg - 300.0))


def in_phase_window(phase_deg: float, low: float = 285.0, high: float = 315.0) -> bool:
...
def in_extended_window(phase_deg: float) -> bool:
    phase = phase_deg % 360.0
    return 270.0 <= phase <= 330.0


def classify_candidate_300(row: dict) -> dict[str, bool]:
    ...
    hard = nearest == 300 and phase_err <= 12 and tx >= 0.75 and ratio >= 20 and matrix <= 0.25 and legal
    preferred = hard and phase_err <= 8 and tx >= 0.85 and ratio >= 30 and matrix <= 0.22
...
def candidate_score_300(row: dict) -> float:
    return 25*flt(row.get("conversion_to_leakage_ratio"),0) + 10*flt(row.get("Tx"),0) - 10*flt(row.get("phase_error_deg"),999) - 60*flt(row.get("matrix_error"),999) - 25*flt(row.get("estimated_leakage"),999)


def normalize_existing_300(row: dict[str,str], source_file: Path) -> dict | None:
    ...
    for pattern in patterns:
        for path in repo_root.glob(pattern):
            for raw in read_csv(path):
                row = normalize_existing_300(raw, path)
                if row is None:
                    continue
                cid = row["candidate_id"]
                if cid not in seen or candidate_score_300(row) > candidate_score_300(seen[cid]):
                    seen[cid] = row
    return sorted(seen.values(), key=lambda r: (int(flt(r.get("nearest_bin_deg"),-999)) != 300, flt(r.get("phase_error_deg"),999), -flt(r.get("conversion_to_leakage_ratio"),0), -flt(r.get("Tx"),0)))


def cluster_existing(rows: list[dict], paths: Stage12_10DPaths) -> list[dict]:
    ...
    for key, group in groups.items():
        ranked = sorted(group, key=lambda r: (int(flt(r.get("nearest_bin_deg"),-999)) != 300, flt(r.get("phase_error_deg"),999), -flt(r.get("conversion_to_leakage_ratio"),0)))
        best = ranked[0]
    ...
    write_csv(paths.output_dir / "stage12_10d_300bin_family_cluster_audit.csv", clusters, CLUSTER_FIELDS)
    lines = ["# Stage12-10D 300-bin Family Cluster Summary", "", f"broad_300_neighborhood_candidate_count = {len(rows)}", f"family_cluster_count = {len(clusters)}", "", "| family | count | preferred-window | best candidate | best ratio | best Tx | best phase error | best matrix error |", "|---|---:|---:|---|---:|---:|---:|---:|"]
    for row in clusters:
    ...
    (paths.output_dir / "stage12_10d_300bin_family_cluster_summary.md").write_text("\n".join(lines)+"\n", encoding="utf-8")
    return clusters
...
def candidate_specs() -> list[tuple[str,bool,float,float,str]]:
    return [
        ("x_pair", True, 70, -30, "x_pair_swap_lowG"),
        ("x_pair", True, 80, -30, "x_pair_swap_frozen_control"),
        ("x_pair", True, 90, -30, "x_pair_swap_highG"),
        ("x_pair", True, 80, -40, "x_pair_swap_farO_low"),
        ("x_pair", True, 80, -20, "x_pair_swap_farO_high"),
        ("x_pair", False, 80, -30, "x_pair_noswap_control"),
        ("x_pair", False, 100, -24, "x_pair_noswap_highG"),
        ("y_pair", True, 80, -30, "y_pair_swap_control"),
        ("y_pair", False, 80, -30, "y_pair_noswap_control"),
        ("diag_pair", True, 80, -30, "diag_pair_swap_control"),
        ("diag_pair", False, 80, -30, "diag_pair_noswap_control"),
        ("diag_pair", True, 100, -20, "diag_pair_swap_highG"),
    ]


def generate_candidate_plan(repo_root: Path, paths: Stage12_10DPaths) -> tuple[list[dict], list[dict]]:
    source = load_plan_row(repo_root, FROZEN_300["candidate_id"])
    rows, rejected = [], []
    for idx, (placement, swap, gap, offset, label) in enumerate(candidate_specs(), 1):
        ...
        cid = f"H500DIMER12D_{idx:03d}_B300_{placement}_{swap_label}_G{int(gap)}_O{int(offset)}"
        if not legal:
            rejected.append({"candidate_id": cid, "reason": "geometry_illegal_or_clearance_below_15nm", "notes": f"clearance={dgap}, edge={edge}"})
            continue
        row = dict(source)
        row.update({"dimer_case_id": cid, "geometry_family": f"{placement}_{swap_label}", "refine_family": f"stage12_10d_{label}", "target_actual_bin_deg": "300", "bin_deg": "300", "static_output_phase_deg": "300", "static_predicted_ratio": "", "static_phase_error_deg": "", "static_target_x_power": "", "static_leak_y_power": "", "height_nm": "500.000000", "lambda_nm": source.get("lambda_nm", "450.000000"), "placement_type": placement, "swap_order": "J2-J1" if swap else "J1-J2", "gap_nm": fmt(float(gap)), "local_offset_nm": fmt(float(offset)), "j1_center_x_nm": fmt(x1), "j1_center_y_nm": fmt(y1), "j2_center_x_nm": fmt(x2), "j2_center_y_nm": fmt(y2), "dimer_gap_nm": fmt(dgap), "edge_margin_nm": fmt(edge), "minimum_clearance_nm": fmt(dgap), "clearance_below_20nm": str(dgap < 20).lower(), "geometry_legal": "true", "run_selected": "true", "notes": "Stage12-10D H500 300-degree single-dimer high-margin refinement; no K=6."})
        rows.append(row)
    rows = rows[:MAX_NEW_CANDIDATES]
    write_csv(paths.output_dir / "stage12_10d_300bin_candidate_plan.csv", rows, PLAN_FIELDS)
    write_csv(paths.output_dir / "stage12_10d_300bin_geometry_audit.csv", rows, PLAN_FIELDS)
    write_csv(paths.output_dir / "stage12_10d_300bin_failed_or_rejected_candidates.csv", rejected, REJECT_FIELDS)
    return rows, rejected


def run_fdtd(repo_root: Path, paths: Stage12_10DPaths, plan_rows: list[dict], runtime_path: str = "configs/runtime.yaml") -> list[dict]:
    runner = load_stage11_runner(repo_root)
    runner.FDTD_DIR = paths.fdtd_work_dir
    runner.RESULT_CSV = paths.output_dir / "stage12_10d_300bin_raw_fdtd_results.csv"
    runner.SUMMARY_MD = paths.output_dir / "stage12_10d_300bin_fdtd_summary.md"
...
def result_to_metric_300(row: dict, plan_by_id: dict[str,dict]) -> dict:
    ...
    out["phase_error_deg"] = phase_error_to_300(phase)
    out["geometry_family"] = plan.get("geometry_family", parse_family(plan or out))
    out.update(classify_candidate_300(out))
    return out


def rank_results(results: list[dict], paths: Stage12_10DPaths) -> list[dict]:
    plan_by_id = {row.get("dimer_case_id"): row for row in read_csv(paths.output_dir / "stage12_10d_300bin_candidate_plan.csv")}
    rows = [result_to_metric_300(row, plan_by_id) for row in results]
    rows = sorted(rows, key=lambda r: (not bool_from(r.get("hard_pass")), int(flt(r.get("nearest_bin_deg"),-999)) != 300, flt(r.get("phase_error_deg"),999), -flt(r.get("conversion_to_leakage_ratio"),0), -flt(r.get("Tx"),0), flt(r.get("matrix_error"),999)))
    fields = ["candidate_id","geometry_family"] + [f for f in RESULT_FIELDS if f != "candidate_id"]
    write_csv(paths.output_dir / "stage12_10d_300bin_fdtd_results.csv", rows, fields)
    write_csv(paths.output_dir / "stage12_10d_300bin_ranked_candidates.csv", rows, fields)
    return rows


def write_summaries(paths: Stage12_10DPaths, ranked: list[dict], cluster_count: int, generated_count: int, fdtd_runs: int) -> dict:
    best = ranked[0] if ranked else {}
    hard = bool_from(best.get("hard_pass")); pref = bool_from(best.get("preferred_pass")); stretch = bool_from(best.get("stretch_pass"))
    improves = bool(best) and flt(best.get("conversion_to_leakage_ratio"),0) > FROZEN_300["ratio"] and flt(best.get("estimated_leakage"),999) < FROZEN_300["estimated_leakage"]
    lines = ["# Stage12-10D 300-bin Best Candidate Summary", "", f"- Family clusters audited: {cluster_count}.", f"- New 300-degree candidates generated: {generated_count}.", f"- Single-source FDTD runs: {fdtd_runs}.", f"- New 300-degree candidate improves over frozen 300-degree bin: {improves}.", f"- Best candidate ID: {best.get('candidate_id','')}.", f"- Best family: {best.get('geometry_family','')}.", f"- Best ratio: {best.get('conversion_to_leakage_ratio','')}.", f"- Best Tx: {best.get('Tx','')}.", f"- Best phase error: {best.get('phase_error_deg','')}.", f"- Best matrix_error: {best.get('matrix_error','')}.", f"- Estimated leakage: {best.get('estimated_leakage','')}.", f"- Hard pass: {hard}.", f"- Preferred pass: {pref}.", f"- Stretch pass: {stretch}.", f"- Use this candidate in a later K=6 replacement test: {hard}.", ("- If 300-degree improves strongly, recommend Stage12-11 K=6 replacement test using improved 300-degree." if hard else "- Since 300-degree also fails, recommend stopping single-bin tuning and moving to broader high-margin six-bin library redesign or reporting current Stage12 baseline as-is."), "- Boundary: single-dimer only; no K=6, no dipoles, no CP, no y-gradient, no H600/H700."]
    (paths.output_dir / "stage12_10d_300bin_best_candidate_summary.md").write_text("\n".join(lines)+"\n", encoding="utf-8")
    rec = ["# Stage12-10D Next Recommendation", "", ("Candidate is eligible for Stage12-11 K=6 replacement validation after an explicit freeze." if hard else "Do not replace the frozen 300-degree bin yet."), "If no hard-pass 300-degree candidate exists, stop single-bin tuning and either redesign the six-bin library for higher margin or report current Stage12 baseline as-is."]
    (paths.output_dir / "stage12_10d_next_recommendation.md").write_text("\n".join(rec)+"\n", encoding="utf-8")
...
def run_stage12_10d(repo_root: Path, paths: Stage12_10DPaths, runtime_path: str = "configs/runtime.yaml") -> dict:
