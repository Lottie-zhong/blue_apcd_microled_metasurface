...
def main() -> None:
    ...
    if group == "POL_ANGLE_BROADBAND" and budget.get("broadband_authorization_status") not in {"AUTHORIZED_STAGE_A_445_455_FROZEN_SPACER_POLARIZATION_ANGLE_BROADBAND", "STAGE_A_445_455_FROZEN_SPACER_POLARIZATION_ANGLE_BROADBAND_RUNNING"}:
    ...
    if group == "POL_ANGLE_BROADBAND":
        if int(budget.get("new_broadband_cases_entered", 0)) >= int(budget.get("new_broadband_cases_budget", 0)):
    ...
    if case_id in completed or case_id in incident_completed or case_id in broadband_completed:
    ...
    if group == "POL_ANGLE_BROADBAND":
        budget["broadband_authorization_status"] = "STAGE_A_445_455_FROZEN_SPACER_POLARIZATION_ANGLE_BROADBAND_RUNNING"
    else:
    ...
    try:
        ...
        if group == "POL_ANGLE_BROADBAND":
            budget["broadband_authorization_status"] = "STAGE_A_445_455_FROZEN_SPACER_POLARIZATION_ANGLE_BROADBAND_RUNNING" if budget["new_broadband_cases_completed"] < budget["new_broadband_cases_budget"] else "STAGE_A_445_455_FROZEN_SPACER_POLARIZATION_ANGLE_BROADBAND_ALL_COMPLETED"
        else:
        ...
        if group == "POL_ANGLE_BROADBAND":
            budget["broadband_authorization_status"] = "STAGE_A_445_455_FROZEN_SPACER_POLARIZATION_ANGLE_BROADBAND_ENTERED_FAILURE_REPLAY_FORBIDDEN"
        else:
