...
def build_fsp(case,prefsp,helper_root):
 import lumapi
 wavelength_start_nm=float(case.get('source_wavelength_start_nm',450.0)); wavelength_stop_nm=float(case.get('source_wavelength_stop_nm',450.0)); frequency_points=int(case.get('frequency_points',1))
 ...
 try:
  ...
  for prop,val in (('injection axis','z-axis'),('direction','Forward'),('polarization angle',0),('x span',1740e-9),('y span',290e-9),('z',-250e-9),('wavelength start',wavelength_start_nm*1e-9),('wavelength stop',wavelength_stop_nm*1e-9)): fdtd.set(prop,val)
  ...
  fdtd.setglobalmonitor('use source limits',1); fdtd.setglobalmonitor('use wavelength spacing',1); fdtd.setglobalmonitor('frequency points',frequency_points); fdtd.save(str(prefsp))
 finally: fdtd.close()
...
def validate(case,rb):
 exp_layers=expected_objects(case,'mdc_layer')+expected_objects(case,'interface_support_layer')+expected_objects(case,'extra_spacer'); exp_p=expected_objects(case,'np_pillar'); got=rb['stack_objects']; expected_monitor_z=((case['coordinates']['np_pillar_top_nm']+400.0)*1e-9 if exp_p else 1875e-9); expected_start=float(case.get('source_wavelength_start_nm',450.0))*1e-9; expected_stop=float(case.get('source_wavelength_stop_nm',450.0))*1e-9; expected_points=int(case.get('frequency_points',1)); expected_step=(expected_stop-expected_start)/(expected_points-1) if expected_points>1 else 0.0
 checks={'dimension_3d':rb['solver']['dimension']=='3D','periodic_xy':all(rb['solver'][key]=='Periodic' for key in ('x min bc','x max bc','y min bc','y max bc')),'pml_z':rb['solver']['z min bc']=='PML' and rb['solver']['z max bc']=='PML','pml_layers':float(rb['solver']['pml layers'])==8.0,'mesh_accuracy':float(rb['solver']['mesh accuracy'])==2.0,'simulation_time':float(rb['solver']['simulation time'])==1e-12,'autoshutoff':float(rb['solver']['auto shutoff min'])==1e-5,'source_x_normal':rb['source']['direction']=='Forward' and float(rb['source']['polarization angle'])==0 and float(rb['source']['angle theta'])==0 and float(rb['source']['angle phi'])==0,'source_wavelength':abs(float(rb['source']['wavelength start'])-expected_start)<1e-15 and abs(float(rb['source']['wavelength stop'])-expected_stop)<1e-15, 'exact_frequency_grid':all(abs(float(rb['monitors'][name]['frequency points'])-expected_points)<1e-9 for name in ('transmission_monitor','reflection_monitor','order_monitor','field_450_monitor')) and (expected_points==1 or abs((expected_stop-expected_start)/(expected_points-1)-1e-9)<1e-18),'stack_presence_absence':len(got)==len(exp_layers) and len(rb['np_pillars'])==len(exp_p),'layer_identity':all(a['material']==b['material_id'] and abs(a['z_max_nm']-a['z_min_nm']-b['thickness_nm'])<1e-6 for a,b in zip(got,exp_layers)),'pillar_identity':all(a['material']==b['material_id'] and abs(a['x_nm']-b['x_nm'])<1e-6 and abs(a['diameter_nm']-b['diameter_nm'])<1e-6 and abs(a['z_max_nm']-a['z_min_nm']-500)<1e-6 for a,b in zip(rb['np_pillars'],exp_p)),'native_materials':all(x['present'] and x['sample_count']>1 for x in rb['materials'].values()),'no_gap_no_overlap':all(abs(a['z_max_nm']-b['z_min_nm'])<1e-6 for a,b in zip(sorted(got,key=lambda x:x['z_min_nm'])[:-1],sorted(got,key=lambda x:x['z_min_nm'])[1:])) and (not got or abs(sorted(got,key=lambda x:x['z_max_nm'])[-1]['z_max_nm']-case['coordinates']['stack_top_nm'])<1e-6),'same_material_continuity':(not expected_objects(case,'extra_spacer')) or (got[-1]['material']=='APCD_SIO2_NATIVE_M1' and abs(got[-1]['z_min_nm']-case['coordinates']['mdc_top_nm'])<1e-6 and abs(got[-1]['z_max_nm']-case['coordinates']['np_pillar_bottom_nm'])<1e-6),'monitor_positions':rb['monitors']['reflection_monitor']['z']==-300e-9 and abs(rb['monitors']['transmission_monitor']['z']-expected_monitor_z)<1e-15 and abs(rb['solver']['z max']-rb['monitors']['transmission_monitor']['z']-300e-9)<1e-15}
...
def main():
 ...
 cfg=json.loads(args.fixture_registry.read_text(encoding='utf-8')); item=next(x for x in cfg['controls'] if x['control_group']==args.control_group); case=build_joint_case(item['mdc_candidate'],item['np_candidate'],item.get('spacer_nm',0),450,'x',0,interface_candidate=item.get('interface_candidate'),case_id=item['case_id'],control_group=item['control_group']); case.update({'source_wavelength_start_nm':item.get('wavelength_start_nm',450.0),'source_wavelength_stop_nm':item.get('wavelength_stop_nm',450.0),'frequency_points':item.get('frequency_points',1),'exact_wavelength_grid_nm':item.get('exact_wavelength_grid_nm',[item.get('wavelength_start_nm',450.0)])}); out=args.output_dir; out.mkdir(parents=True,exist_ok=True); write(out/'joint_case.json',case); setup=out/'setup'; setup.mkdir(exist_ok=True); pre=setup/f"{case['case_id']}_pre.fsp"; build_fsp(case,pre,args.material_helper_root); rb=readback(case,pre,args.material_helper_root); gate=validate(case,rb); manifest={'schema_version':'stage_a_control_setup_manifest_v1','case_id':case['case_id'],'control_group':args.control_group,'case':case,'readback':rb,'setup_gate':gate,'pre_fsp_path':str(pre),'pre_fsp_sha256':sha256(pre),'solver_entered':False,'solver_completed':False,'source_commits':cfg['source_commits'],'coupling_commit':subprocess.check_output(['git','-C',str(ROOT),'rev-parse','HEAD'],text=True).strip()}; write(out/'setup_manifest.json',manifest); write(out/'setup_readback.json',rb); write(out/'setup_gate.json',gate); print(json.dumps({'case_id':case['case_id'],'control_group':args.control_group,'setup_gate':gate,'pre_fsp_sha256':manifest['pre_fsp_sha256']},indent=2));
