...
def build_fsp(case,prefsp,helper_root):
 ...
 try:
  ...
  fdtd.addfdtd()
  for prop,val in (('dimension','3D'),('x span',1740e-9),('y span',290e-9),('z min',-600e-9),('z max',2175e-9),('x min bc','Periodic'),('x max bc','Periodic'),('y min bc','Periodic'),('y max bc','Periodic'),('z min bc','PML'),('z max bc','PML'),('mesh accuracy',2),('pml layers',8),('simulation time',1e-12),('auto shutoff min',1e-5),('dt stability factor',0.99)): fdtd.set(prop,val)
  ...
  for role,prefix in (('mdc_layer','MDC_layer'),('interface_support_layer','Support_layer')):
   for obj in expected_objects(case,role):
    fdtd.addrect(); fdtd.set('name',f"{prefix}_{int(obj['index']):02d}"); fdtd.set('x span',1740e-9); fdtd.set('y span',290e-9); fdtd.set('z min',obj['z_min_nm']*1e-9); fdtd.set('z max',obj['z_max_nm']*1e-9); fdtd.set('material',obj['material_id'])
...
def readback(case,prefsp,helper_root):
 ...
 try:
  ...
  for role,prefix in (('mdc_layer','MDC_layer'),('interface_support_layer','Support_layer')):
   for obj in expected_objects(case,role):
    name=f"{prefix}_{int(obj['index']):02d}"; stack.append({'role':role,'name':name,'material':one(fdtd,name,'material'),'z_min_nm':one(fdtd,name,'z min')*1e9,'z_max_nm':one(fdtd,name,'z max')*1e9})
  pillars=[]
  for obj in expected_objects(case,'np_pillar'):
