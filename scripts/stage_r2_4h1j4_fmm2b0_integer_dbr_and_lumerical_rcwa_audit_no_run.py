...
RUNTIME_DIR = ROOT / "runtime" / "r2_4h1j4_rcled_mdc_integer_dbr_derived_fsp_DO_NOT_COMMIT"
DERIVED_FSP = RUNTIME_DIR / "MDC_blue_oujizi_RCLED_QWinteger453_10pair_H1J4.fsp"
OUT = ROOT / "outputs" / "r2_4h1j4_fmm2b0_integer_dbr_and_lumerical_rcwa_audit_no_run"
...
GROUP = "H1J4_bottom_DBR_QWinteger453_10pair"
TIO2_NM = 45.0
SIO2_NM = 79.0
PAIR_COUNT = 10
DBR_Y_MAX_NM = -950.0
...
h1j4_decision = "integer_derived_fsp_creation_failed_no_run"
...
try:
    ...
    for pair in range(1, PAIR_COUNT + 1):
        for mat_label, mat_name, thickness_nm in [("TiO2", "tio22", TIO2_NM), ("SiO2", "sio222", SIO2_NM)]:
            ...
            child_name = f"H1J4_bottom_DBR_layer_{idx:02d}_{mat_label}"
            fdtd.addrect()
    ...
    creation_rows.append({"step": "save_derived_h1j4_fsp", "status": "ok" if save_succeeded else "failed", "detail": str(DERIVED_FSP)})
    ...
    if save_succeeded:
        if mesh_order_status.startswith("verified") and farfield_settings_status.startswith("programmatically_accessible"):
            h1j4_decision = "integer_derived_runtime_fsp_created_for_manual_gui_audit"
        else:
            h1j4_decision = "integer_derived_runtime_fsp_created_requires_manual_gui_audit"
    else:
        h1j4_decision = "integer_derived_fsp_creation_failed_no_run"
except Exception as exc:
...
# Write outputs.
write_csv(OUT / "r2_4h1j4_derived_fsp_creation_status.csv", creation_rows + errors)
write_csv(OUT / "r2_4h1j4_bottom_dbr_group_record.csv", group_rows)
write_csv(OUT / "r2_4h1j4_bottom_dbr_layer_table.csv", layer_rows)
write_csv(OUT / "r2_4h1j4_mesh_order_overlap_fix_record.csv", mesh_rows)
write_csv(OUT / "r2_4h1j4_fdtd_region_record.csv", fdtd_rows)
write_csv(OUT / "r2_4h1j4_source_record.csv", source_rows)
write_csv(OUT / "r2_4h1j4_monitor_global_coordinate_record.csv", monitor_rows)
write_csv(OUT / "r2_4h1j4_farfield_settings_record.csv", farfield_rows)
write_csv(OUT / "r2_4h1j4_geometry_safety_audit.csv", safety_rows)

write_md(OUT / "r2_4h1j4_manual_gui_audit_checklist.md", f"""
# R2-4H1J4 manual GUI audit checklist

Open the H1J4 derived FSP:
...
- TiO2 layers are about 45 nm
- SiO2 layers are about 79 nm
- all bottom DBR layers use object-level mesh order override = true and mesh order = 1
- bottom DBR y range is about -2190 nm to -950 nm
...
write_md(OUT / "r2_4h1j4_do_not_commit_runtime_fsp_note.md", f"""
# Do not commit H1J4 runtime FSP

The H1J4 derived FSP is a runtime artifact only:
...
write_md(OUT / "r2_4h1j4_next_stage_recommendation.md", """
# R2-4H1J4 next-stage recommendation

Next stage: `H1K manual GUI audit record for H1J4 derived FSP, no FDTD`.
...
write_md(OUT / "r2_4h1j4_stop_allow_rules.md", """
# R2-4H1J4 stop / allow rules
...
write_md(OUT / "r2_4h1j4_summary.md", f"""
# R2-4H1J4 summary

Decision: `{h1j4_decision}`.
...
manifest = {
    ...
    "outputs": sorted(set([p.name for p in OUT.iterdir() if p.is_file()] + ["r2_4h1j4_manifest.json"])),
}
(OUT / "r2_4h1j4_manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
...
# H1J4/FMM2B0 compact appended no-run RCWA audit and required joint outputs.
def append_rcwa_and_joint_outputs():
    rows=[]
    def rec(n,m,st,d,c=""):
        rows.append({"probe_name":n,"probe_method":m,"status":st,"detail":str(d)[:1000],"classification":c})
    dec="lumerical_rcwa_unavailable_or_unconfirmed"; avail=False; ready=False
    try:
        lm=load_lumapi(); f=lm.FDTD(hide=True); rec("open_lumerical_hidden","lumapi.FDTD(hide=True)","ok","session opened","session_available")
        try:
            f.addrcwa(); rec("addrcwa_object_creation","fdtd.addrcwa()","ok","RCWA object created; no solve called","rcwa_available_for_api_probe"); dec="lumerical_rcwa_available_for_minimal_probe"; avail=True; ready=True
        except Exception as exc:
            rec("addrcwa_object_creation","fdtd.addrcwa()","failed",f"{type(exc).__name__}: {exc}","rcwa_unavailable_or_license_blocked")
        for cmd in ["?addrcwa;","?rcwa;"]:
            try:
                f.eval(cmd); rec(cmd.strip("?;")+"_help_query",f"eval('{cmd}')","ok","recognized; no calculation called","rcwa_script_command_possible")
            except Exception as exc:
                rec(cmd.strip("?; ")+"_help_query",f"eval('{cmd}')","failed",f"{type(exc).__name__}: {exc}","rcwa_unknown_or_unavailable")
        try: f.close()
        except Exception: pass
    except Exception as exc:
        rec("open_lumerical_hidden","lumapi.FDTD(hide=True)","failed",f"{type(exc).__name__}: {exc}","license_or_api_unavailable")
    write_csv(OUT/"r2_fmm2b0_rcwa_probe_attempt_log.csv",rows)
    write_csv(OUT/"r2_fmm2b0_lumerical_rcwa_availability_audit.csv",[{"rcwa_audit_decision":dec,"addrcwa_available_for_api_probe":avail,"fmm_ready_for_minimal_probe":ready,"calculation_run":False,"project_file_saved":False,"notes":"Availability probe only; no RCWA/FMM calculation or sweep was run."}])
    write_csv(OUT/"r2_4h1j4_integer_dbr_design_rationale.csv",[{"candidate_name":"DBR_QWinteger453_10pair","target_wavelength_nm":453,"pair_count":10,"tio2_integer_nm":45,"sio2_integer_nm":79,"tio2_reference_nm":44.7,"sio2_reference_nm":79.4,"total_thickness_nm":1240,"rationale":"Closest integer approximation to 453 nm quarter-wave values with minimal optical-thickness change."}])
    write_md(OUT/"r2_fmm2b0_rcwa_next_probe_plan.md",f"# R2-FMM2B0 RCWA Next Probe Plan\n\nDecision: `{dec}`. If `addrcwa` was confirmed, FMM2B1 may be a minimal API smoke test only after explicit user approval. No RCWA/FMM sweep is allowed here.")
    write_md(OUT/"r2_4h1j4_manual_gui_audit_checklist.md",f"# R2-4H1J4 Manual GUI Audit Checklist\n\nOpen `{DERIVED_FSP}` and confirm group `{GROUP}`, 20 layers, TiO2/SiO2 order, 45/79 nm thicknesses, y range about -2190 to -950 nm, mesh order override=true and mesh order=1, source_1 x=0 nm y=-800 nm z=0 nm wavelength 453 nm theta=90 phi=0 enabled=true, PlaneSource disabled, FDTD y range -2800 to +1400 nm, monitor y=1100 nm on output side and outside stack/PML, approved far-field settings, no simulation run, and FSP not overwritten.")
    write_md(OUT/"r2_4h1j4_fmm2b0_joint_decision.md",f"# R2-4H1J4 / FMM2B0 Joint Decision\n\n`h1j4_decision = {h1j4_decision}`\n\n`rcwa_audit_decision = {dec}`\n\n`fmm_ready_for_minimal_probe = {str(ready).lower()}`\n\nImmediate FDTD and RCWA/FMM calculations are not allowed.")
    write_md(OUT/"r2_4h1j4_fmm2b0_next_stage_recommendation.md","# Next Stage Recommendation\n\nIf H1J4 GUI audit passes, record it in H1K4 without FDTD. If RCWA availability is confirmed, FMM2B1 can be a minimal no-sweep API smoke test after explicit approval. If GUI fails, do H1J5 fix first.")
    write_md(OUT/"r2_4h1j4_fmm2b0_stop_allow_rules.md","# Stop / Allow Rules\n\nStop: no FDTD, no RCWA/FMM calculation, no run/runanalysis/mesh/optimize/sweep, no y-dipole, no broadband, no APCD, no committing heavy files. Allow: create derived runtime FSP, no-run RCWA availability probe, lightweight reports and runtime .gitignore.")
    write_md(OUT/"r2_4h1j4_fmm2b0_summary.md",f"# R2-4H1J4 FMM2B0 Summary\n\nDerived FSP: `{DERIVED_FSP}`. H1J4 decision `{h1j4_decision}`. Bottom group `{GROUP}`, layers `{len(layer_rows)}`, TiO2/SiO2 `{TIO2_NM}/{SIO2_NM}` nm, y range `{DBR_Y_MIN_NM}` to `{DBR_Y_MAX_NM}` nm, mesh `{mesh_order_status}`, monitor safety `{monitor_global_safety}`, far-field `{farfield_settings_status}`, RCWA `{dec}`, FMM ready `{str(ready).lower()}`. Manual GUI audit is required next.")
    old=OUT/"r2_4h1j4_manifest.json"; manifest=json.loads(old.read_text(encoding="utf-8")) if old.exists() else {}
    manifest.update({"stage":"R2-4H1J4_FMM2B0_integer_DBR_and_Lumerical_RCWA_audit_no_run","candidate_name":"DBR_QWinteger453_10pair","rcwa_audit_decision":dec,"lumerical_addrcwa_available":avail,"fmm_ready_for_minimal_probe":ready,"immediate_rcwa_fmm_calculation_allowed":False,"immediate_heavy_fmm_sweep_allowed":False,"manual_gui_audit_required_next":True,"outputs":sorted(p.name for p in OUT.iterdir() if p.is_file())})
    (OUT/"r2_4h1j4_fmm2b0_manifest.json").write_text(json.dumps(manifest,indent=2),encoding="utf-8")
    print(json.dumps({"h1j4_decision":h1j4_decision,"derived_fsp_exists":DERIVED_FSP.exists(),"rcwa_audit_decision":dec,"lumerical_addrcwa_available":avail,"fmm_ready_for_minimal_probe":ready,"immediate_fdtd_allowed":False,"immediate_rcwa_fmm_calculation_allowed":False},indent=2))
append_rcwa_and_joint_outputs()
