...
C3 = O / "clean_v3"
A = O / "analysis"
P = O / "plans/lp_ml_six_bin_inverse_search_round3_v1"
REPORT = ROOT / "reports/lp_ml_round3_six_bin_inverse_search_v1.md"
...
def load_models(mod, kind, old_mu, old_sd, clean_mu, clean_sd, c5_mu=None, c5_sd=None):
    if kind == "C0":
        root = O / "model_runtime_round1_frozen_v1"
        mu, sd = old_mu, old_sd
    elif kind == "C5":
        root = C3 / "model_runtime_round3_c5_v1"
        mu, sd = c5_mu, c5_sd
    else:
...
def planning_prediction(models, u, waves):
    c0, _, _ = predict_kind(models["C0"][0], models["C0"][1], models["C0"][2], u, waves)
    c1, _, c1s = predict_kind(models["C1"][0], models["C1"][1], models["C1"][2], u, waves)
    c5, _, c5s = predict_kind(models["C5"][0], models["C5"][1], models["C5"][2], u, waves)
    old = 0.95*c0 + 0.05*c1
    blend = 0.95*old + 0.05*c5
    return c0, blend, torch.cat([c1s, c5s], dim=0)
...
def optimize_batch(models, old_mu, old_sd, clean_mu, clean_sd, starts, target_phase, scales, weights, steps=28):
    ...
    for _ in range(steps):
        ...
        c0, blend, c1_stack = planning_prediction(models, u, waves)
...
def eval_candidates(models, vectors, target_phases, offsets, sources, traces, scales, weights):
    ...
    preds = {}
    for kind in ["C0", "C1", "C2", "C3", "C4", "C5"]:
        preds[kind] = predict_kind(models[kind][0], models[kind][1], models[kind][2], u, waves)
    old = 0.95 * preds["C0"][0] + 0.05 * preds["C1"][0]
    blend = 0.95 * old + 0.05 * preds["C5"][0]
    rows = []
    for i, v in enumerate(vectors):
        ok, direct, periodic = manufacturing(v)
        c0 = preds["C0"][0][i : i + 1]
        b = blend[i : i + 1]
        c1stack = torch.cat([preds["C1"][2][:, i : i + 1], preds["C5"][2][:, i : i + 1]], dim=0)
...
def main():
    ...
    c5_norm = json.loads((C3 / "normalization_clean_v3.json").read_text())
    c5_mu = np.array(c5_norm["mean"], dtype=np.float32); c5_sd = np.array(c5_norm["std"], dtype=np.float32)
    models = {k: load_models(mod, k, old_mu, old_sd, clean_mu, clean_sd, c5_mu, c5_sd) for k in ["C0", "C1", "C2", "C3", "C4", "C5"]}
    ...
    blend_center = 0.95 * (0.95 * model_center["C0"] + 0.05 * model_center["C1"]) + 0.05 * model_center["C5"]
    ...
    val_ensemble = torch.stack([val_pred_means[k] for k in ["C1", "C2", "C3", "C4", "C5"]])
    val_risk = torch.sqrt(torch.mean((val_ensemble - val_ensemble.mean(0)) ** 2, dim=(0, 2, 3))).detach().cpu().numpy()
    val_blend = 0.95 * (0.95 * val_pred_means["C0"] + 0.05 * val_pred_means["C1"]) + 0.05 * val_pred_means["C5"]
    ...
    manifest.update({
        "contract_version": "LP_ML_ROUND3_SIX_BIN_INVERSE_EXECUTION_MANIFEST_V1",
        "clean_v3_dataset_sha256": sha(C3 / "lp_ml_dataset_v1_merged_clean_v3_round3_377_geometry_3393_rows.csv"),
        "clean_v3_normalization_sha256": sha(C3 / "normalization_clean_v3.json"),
        "validation_selection_sha256": sha(A / "lp_ml_round3_validation_selection_v1.json"),
        "planning_model": "OLD_C5_BLEND_0.95",
        "planning_weights": {"C0": 0.9025, "C1": 0.0475, "C5": 0.05},
    })
    ...
    report = report.replace("C0 is the global guard; the alpha=0.95 C0/C1 blend is the primary differentiable model; C1-C4/seed ensembles provide disagreement diagnostics.", "C0 is the global guard; validation-frozen OLD_C5_BLEND_0.95 (weights C0=0.9025, C1=0.0475, C5=0.05) is the planning model; C1-C5/seed ensembles provide disagreement diagnostics.")
    report = report.replace("No geometry 054, no Round-3, no inverse FDTD, no K6, no six-bin promotion, no frozen-test tuning, no model retraining, and no new physics.", "No geometry 054, no inverse FDTD, no K6, no six-bin promotion, no frozen-test tuning, and no new physics. Round-3 C5 was trained offline from scratch before this search; coverage is 58/64 complete geometries.")
