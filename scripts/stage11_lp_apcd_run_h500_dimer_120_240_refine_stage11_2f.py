...
RESULT = OUT_DIR / "h500_dimer_120_240_refine_fdtd_results_stage11_2f.csv"
SUMMARY = OUT_DIR / "h500_dimer_120_240_refine_fdtd_summary_stage11_2f.md"
FDTD_DIR = OUT_DIR / "fdtd_h500_dimer_120_240_refine"
...
def map_row(row: dict[str, str]) -> dict[str, str]:
    return {
        **row,
        "bin_deg": row["target_actual_bin_deg"],
        "static_output_phase_deg": row["target_actual_bin_deg"],
...
def write_summary(rows: list[dict[str, str]], dry: bool = False) -> None:
    ...
    lines = [
        "# Stage11-2F H500 120/240 Refine FDTD Summary",
        "",
        f"mode = {'dry_run_no_lumerical' if dry else 'real_h500_120_240_refine_fdtd'}",
...
def main() -> int:
    ...
    rows = [
        map_row(row)
        for row in mod.read_csv(Path(args.plan))
        if row.get("geometry_legal") == "true"
    ][: args.max_cases]

    print(f"selected_legal_120_240_refine_count={len(rows)}")
    for row in rows[: min(12, len(rows))]:
        print(
            f"case={row['dimer_case_id']} target={row['bin_deg']} "
            f"placement={row['placement_type']} swap={row.get('swap_order', '')} "
            f"gap={row.get('gap_nm', '')} offset={row.get('local_offset_nm', '')} x/y"
        )
    if args.dry_run:
        dry_rows = [
            {**{key: row.get(key, "") for key in mod.RESULT_FIELDS}, "fdtd_status": "dry_run"}
            for row in rows
        ]
