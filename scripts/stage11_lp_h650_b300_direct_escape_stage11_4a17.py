...
RESULT_CSV = REPORT_DIR / "stage11_4a17_h650_b300_direct_escape_results.csv"
RANKING_CSV = REPORT_DIR / "stage11_4a17_h650_b300_direct_escape_ranking.csv"
SUMMARY_JSON = REPORT_DIR / "stage11_4a17_h650_b300_direct_escape_summary.json"
REPORT_MD = REPORT_DIR / "stage11_4a17_h650_b300_direct_escape_report.md"
NEXT_MD = REPORT_DIR / "stage11_4a17_h650_b300_direct_escape_recommended_next.md"
A16_MANIFEST = REPORT_DIR / "stage11_4a16_h650_b300_escape_hatch_manifest.json"
SOURCE_MANIFEST_3B4 = REPORT_DIR / "stage11_3b4_lp_h500_case_manifest.json"
TEMP_FDTD_DIR = REPORT_DIR / "stage11_4a17_fdtd_tmp"
RAW_TMP_CSV = REPORT_DIR / "stage11_4a17_raw_combined_tmp.csv"
RAW_TMP_MD = REPORT_DIR / "stage11_4a17_raw_summary_tmp.md"
...
def run_matrix() -> list[dict[str, str]]:
    ...
    for idx, base in enumerate(geometries(), start=1):
        for wl in WAVELENGTHS:
            ...
            cid = f"H650B300ESCAPE_{idx:03d}_{base['placement_type']}_{base['swap_order'].replace('-', '')}_G{gap}"
            case_id = f"S11_4A17_H650_B300_G1_{idx:03d}_WL{wl}"
            row = dict(base)
            row.update({
                "case_id": case_id,
                ...
                "notes": "Stage11-4A17 A16 G1 H650 B300 direct escape only; no G2, coverage, H600 rerun, K6.",
            })
            rows.append(row)
    if len(rows) != 12:
...
def write_reports(rows: list[dict[str, str]], counts: dict[str, int]) -> None:
    ...
    if strict or loose:
        ...
        next_step = "Record H650 donor reassignment; do not force this family as true B300."
    else:
        next_step = "Stop LP-Hnew six-bin attempt and write route-positioning audit; A17 G1 found no H650 true-B300 near-miss."
    summary = {"stage": "Stage11-4A17", "group_id": GROUP_ID, **counts, "strict_candidates": len(strict), "loose_candidates": len(loose), "near_miss_candidates": len(near), "reassigned_candidates": len(reassigned), "best_candidate": best, "a8_b60_donor_reference": A8_B60_DONOR, "recommended_next": next_step, "no_g2": True, "no_coverage": True, "no_h600_rerun": True, "no_k6": True}
    SUMMARY_JSON.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    REPORT_MD.write_text("\n".join(["# Stage11-4A17 H650 B300 Direct Escape Scout", "", "Scope: only A16 G1, 12 H650 B300 direct escape cases. No G2, coverage, H600 rerun, K=6, H500 rescue.", "", f"planned = {counts['planned']}", f"run = {counts['run']}", f"failed = {counts['failed']}", f"missing = {counts['missing']}", f"strict = {len(strict)}", f"loose = {len(loose)}", f"near_miss = {len(near)}", f"reassigned = {len(reassigned)}", "", "## Best Candidate", "", "```json", json.dumps(best, indent=2), "```", "", "## A8 B60 Donor Reference", "", json.dumps(A8_B60_DONOR, indent=2), "", "## Recommendation", "", next_step, ""]), encoding="utf-8")
    NEXT_MD.write_text("# Stage11-4A17 Recommended Next\n\n" + next_step + "\n", encoding="utf-8")
