#!/usr/bin/env python3
"""R2-4E2 tri-point x-dipole FDTD guard for E1_0236 only.
...
OUT = ROOT / "outputs" / "r2_4e2_e1_0236_tri_point_xdipole_fdtd_guard"
SOLVE_DIR = OUT / "runtime_solve_fsp"
LUMAPI_DIR = Path(r"N:\Program Files\ANSYS Inc\v251\Lumerical\api\python")

STAGE = "R2-4E2 E1_0236 tri-point x-dipole 453 nm FDTD guard"
CANDIDATE_ID = "E1_0236"
FAMILY_ID = "E0C_MQW_lateral_extent_robust_cavity"
WAVELENGTH_NM = 453.0
X_POSITIONS_UM = [-0.7, 0.0, 0.7]
EXPECTED_CASE_IDS = [
    "E1_0236_xm0p7_xdipole_453",
    "E1_0236_x0p0_xdipole_453",
    "E1_0236_xp0p7_xdipole_453",
]

TOP_PAIR_COUNT = 6
BOTTOM_PAIR_COUNT = 6
CAVITY_NM = 260.0
TOP_TERMINATION_NM = 45.0
BOTTOM_TERMINATION_NM = 75.0
...
def make_layers() -> dict[str, list[dict[str, object]]]:
    ...
    bottom: list[dict[str, object]] = [{"layer_index": 0, "material": "SiO2_termination", "thickness_nm": BOTTOM_TERMINATION_NM}]
    idx = 1
...
def build_case(fdtd: Any, x_um: float, stacks: dict[str, list[dict[str, object]]]) -> dict[str, object]:
    ...
    case_id = f"E1_0236_{suffix}_xdipole_453"
...
def main() -> int:
    ...
    for x in X_POSITIONS_UM:
        case_manifest.append({"case_id": f"E1_0236_{case_suffix(x)}_xdipole_453", "source_x_um": x, "candidate_id": CANDIDATE_ID, "dipole_orientation": "x", "wavelength_nm": WAVELENGTH_NM})
    ...
    write_csv(OUT / "r2_4e2_case_manifest.csv", case_manifest)
    ...
    write_csv(OUT / "r2_4e2_case_results.csv", rows)
    ...
    if len(ok_rows) == 3 and len(cuts) == 3:
        ...
        hard_fail = False
        if float(avg_row["tri_point_avg_peak_abs_angle_deg"]) > 10:
            hard_fail = True; verdict_reasons.append("tri_point_avg_peak_abs_angle_gt_10deg")
        ...
        write_csv(OUT / "r2_4e2_angle_cut_data.csv", angle_rows)
    else:
    ...
    write_csv(OUT / "r2_4e2_tri_point_incoherent_average.csv", avg_rows)
    ...
    write_json(OUT / "r2_4e2_manifest.json", manifest)

    summary = [
        "# R2-4E2 E1_0236 Tri-Point X-Dipole FDTD Guard",
        "",
        f"Completed ok cases: {len(ok_rows)} / 3.",
        f"Verdict: **{verdict}**.",
        "",
        "Scope was restricted to E1_0236, x-dipole only, 453 nm only, x = [-0.7, 0, +0.7] um.",
    ...
    write_text(OUT / "r2_4e2_summary.md", "\n".join(summary))

    diag = ["# R2-4E2 Source Position Stability Diagnosis", "", f"Verdict: {verdict}.", ""]
    if avg_rows:
        ar = avg_rows[0]
    ...
    write_text(OUT / "r2_4e2_source_position_stability_diagnosis.md", "\n".join(diag))

    verdict_md = ["# R2-4E2 Pass/Fail Verdict", "", f"Verdict: **{verdict}**.", "", "Rules applied:", "- hard fail if any case status != ok", "- hard fail if tri-point average peak_abs_angle > 10 deg", "- hard fail if tri-point average normal/offaxis <= 1", "- hard fail if any source revives a dominant 30-40 deg lobe", "", f"Reasons: {avg_rows[0].get('verdict_reasons','missing') if avg_rows else 'missing'}"]
    write_text(OUT / "r2_4e2_pass_fail_verdict.md", "\n".join(verdict_md))

    if verdict == "pass":
        next_step = "E1_0236 passes tri-point guard. Next allowed step is a 5-point x-line x-dipole 453 nm plan only; do not execute it in E2."
    else:
        next_step = "E1_0236 fails or did not complete tri-point guard. Stop this candidate; do not continue to 5-point, 9-point, y/z dipole, or broadband FDTD."
    write_text(OUT / "r2_4e2_next_step_plan.md", "# R2-4E2 Next Step Plan\n\n" + next_step)
