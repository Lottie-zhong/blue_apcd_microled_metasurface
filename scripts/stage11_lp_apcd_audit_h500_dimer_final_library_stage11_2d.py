...
IN_FILES=[REPO_ROOT/'outputs/blue10k6_lp_apcd_stage11_2a_h500_dimer_validation/h500_dimer_fdtd_results_stage11_2a.csv',REPO_ROOT/'outputs/blue10k6_lp_apcd_stage11_2b_h500_dimer_rebin/h500_dimer_placement_patch_fdtd_results_stage11_2b.csv',REPO_ROOT/'outputs/blue10k6_lp_apcd_stage11_2c_h500_dimer_alt_pairs/h500_dimer_alt_patch_fdtd_results_stage11_2c.csv',REPO_ROOT/'outputs/blue10k6_lp_apcd_stage11_2d_h500_dimer_final_gap/h500_dimer_final_gap_fdtd_results_stage11_2d.csv']
OUT_DIR=REPO_ROOT/'outputs/blue10k6_lp_apcd_stage11_2d_h500_dimer_final_gap'
SUMMARY=OUT_DIR/'h500_dimer_actual_6bin_summary_stage11_2d.md'; BEST=OUT_DIR/'h500_dimer_best_actual_6bin_library_stage11_2d.csv'; GAP=OUT_DIR/'h500_dimer_remaining_gap_diagnosis_stage11_2d.md'; PROJ=OUT_DIR/'h500_dimer_projection_matrix_diagnosis_stage11_2d.md'
PHASE_BINS=[0,60,120,180,240,300]; EPS=1e-12
FIELDS=['bin_deg','candidate_count','best_case_id','projection_selectivity_ratio','selected_power','blocked_input_total_power','selected_polarization_purity','matrix_projection_error_norm','actual_common_phase_deg','actual_common_phase_error_deg','usable_status','failure_mode','notes']
def read_csv(p):
...
def metric(r):
    ...
    selected=tx*tx; blocked=txy*txy+tyy*tyy; purity=selected/max(selected+tyx*tyx,EPS); ratio=selected/max(blocked,EPS); err=math.sqrt(tyx*tyx+txy*txy+tyy*tyy)/max(tx,EPS); phase=f(r.get('t_xx_phase_deg',r.get('dimer_output_phase_deg'))); b,e=nearest(phase)
...
def main():
    ...
    rows=[]
    for p in IN_FILES:
        for r in read_csv(p): rows.append(metric({**r,'source_file':p.name}))
    best=[]; lines=['# Stage11-2D H500 Final Actual Dimer Projection-Phase 6-bin Summary','', 'Merged 2A + 2B + 2C + 2D real H500 dimer FDTD. Bins use selected-channel actual common phase angle(t_xx).','', '| bin_deg | candidate_count | best_case_id | projection_selectivity_ratio | selected_power | blocked_input_total_power | selected_polarization_purity | matrix_projection_error_norm | actual_common_phase_deg | actual_common_phase_error_deg | usable_status | failure_mode |','|---:|---:|---|---:|---:|---:|---:|---:|---:|---:|---|---|']
    for b in PHASE_BINS:
