...
MANIFEST_PATH = REPORT / "j1_anisotropy_candidate_manifest.json"
CASE_ROOT = ROOT / "outputs/lp_h1f4b1_j1_anisotropy_fullk6_compensator_probe/cases"
ACCOUNTING_PATH = REPORT / "h1f4b1_solver_accounting.json"
POLS = ("x", "y")
...
def manifest():
    ...
    if m.get("status") != "FROZEN_READY_FOR_SOLVER" or m.get("candidate_count") != 2:
        raise RuntimeError("HARD_GATE_H1F4B1_MANIFEST_NOT_FROZEN")
    if m.get("max_new_formal_cases") != 4 or m.get("processes") != 4 or m.get("threads") != 1:
        raise RuntimeError("HARD_GATE_H1F4A_PHASE2_BUDGET_DRIFT")
    if m.get("wavelength_grid_nm") != GRID or m.get("ml_admitted") is not False or m.get("delta_J1_nm") != 2.0:
        raise RuntimeError("HARD_GATE_H1F4B1_CONTRACT_DRIFT")
    cs = candidates(m)
    if len(cs) != 2 or not {c.get("candidate_uid") for c in cs} == {"H1F4B1_K6_L1_C_POS_PLUS10_J1_ANISO_PLUS", "H1F4B1_K6_L1_C_POS_PLUS10_J1_ANISO_MINUS"}:
        raise RuntimeError("HARD_GATE_H1F4B1_CHILDREN")
    for c in cs:
        if c.get("grouped_d_perturbation_nm") != 0.0 or c.get("j1_anisotropy_mode") != "J1_length=J1_side+delta_nm; J1_width=J1_side-delta_nm; preserve mean dimension and all other geometry; same local-axis convention at all six sites" or c.get("helper_J3") is not None:
            raise RuntimeError("HARD_GATE_H1F4B1_MODE_DRIFT")
        if c.get("P_supercell_nm") != 2591.446716 or c.get("P_y_nm") != 432.0:
            raise RuntimeError("HARD_GATE_H1F4B1_PERIOD_DRIFT")
        if not c.get("geometry_legality", {}).get("pass"):
            raise RuntimeError("HARD_GATE_H1F4B1_ILLEGAL_CHILD")
        if c.get("base_candidate_uid") != "K6_L1_C_POS_PLUS10" or c.get("base_candidate_hash") != "a8606d8f44a4675db08493c3dd95c8ea43f30882d3a9bbb18a65b59c2ba45198":
            raise RuntimeError("HARD_GATE_H1F4B1_PARENT_HASH_DRIFT")
    if m.get("parent_hash") != "a8606d8f44a4675db08493c3dd95c8ea43f30882d3a9bbb18a65b59c2ba45198":
        raise RuntimeError("HARD_GATE_H1F4B1_PRIMARY_HASH_DRIFT")
    return m


def identity(candidate, pol):
    m = manifest()
    return {"case_uid": f"{candidate['candidate_uid']}_{pol}", "candidate_uid": candidate["candidate_uid"], "candidate_hash": candidate["candidate_hash"], "base_candidate_uid": candidate["base_candidate_uid"], "j1_delta_nm": candidate["delta_J1_nm"], "j1_anisotropy_mode": candidate["j1_anisotropy_mode"], "grouped_d_perturbation_nm": candidate["grouped_d_perturbation_nm"], "polarization": pol, "manifest_freeze_sha256": m["freeze_sha256"], "wavelength_grid_nm": GRID, "solver_runs_for_spectrum": 1}
...
def postprocess(m):
    ...
    write_csv(REPORT / "h1f4b1_order_resolved_fullwave.csv", rows)
    jrows = []
    for c in candidates(m):
...
def execute():
    ...
    for c in candidates(m):
        for pol in POLS:
            ...
            ledger = read_json(REPORT / "h1f4b1_solver_ledger.json")
            ...
            BASE.write_json(REPORT / "h1f4b1_solver_ledger.json", ledger)
