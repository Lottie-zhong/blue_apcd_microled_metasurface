...
def main():
    ensure_dirs(); created=datetime.datetime.utcnow().isoformat()+'Z'; creation_commit=git('git rev-parse HEAD')
    ...
    ledger_obj={'created_utc':created,'creation_code_commit':creation_commit,'formal_phase_definition':'arg(txx)=atan2(Im(txx),Re(txx)) in degrees modulo 360','matrix_payload_sha256':matrix_payload_sha,'matrix_payload_expected_sha256':EXPECTED_MATRIX_SHA,'matrix_payload_hash_pass':matrix_payload_sha==EXPECTED_MATRIX_SHA,'formal_contract_file_sha256':contract_sha,'formal_contract_expected_sha256':EXPECTED_CONTRACT_SHA,'formal_contract_hash_pass':contract_sha==EXPECTED_CONTRACT_SHA,'formal_target':'P_APCD=diag(1,0)','historical_hard_gate':'HARD_GATE_FROZEN_TXX_REPRODUCTION_FAILURE','sources':ledger,'admitted_unique_geometry_count':len(admitted),'geometry054_admitted_rows':sum(1 for r in admitted if is054(r)),'solver_calls':0,'protected_hashes':protected_hashes}
