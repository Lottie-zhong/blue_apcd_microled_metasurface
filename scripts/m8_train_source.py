...
def load():
 ...
 if len(h)!=440 or len(l)!=440:raise RuntimeError('authority_row_count')
 ...
 if len(geos)!=20:raise RuntimeError('geometry_count')
 for g in geos:
...
def main():
 prereg=OUT/'NP_K6_M8_20G_FORWARD_RETRAINING_PREREG_V1.json';ph=sha(prereg);rec=jread(OUT/'preregistration_sha256.json')
 ...
 print(json.dumps({'event':'m8_training_complete','folds':20,'models':len(MODELS),'seeds':SEEDS,'solver_calls':0},separators=(',',':')))
 ...
 wcsv(OUT/'oof_predictions_20g.csv',prow)
 # residual structure
 rr=[]
 ...
 old=rdcsv(M7OUT/'oof_predictions_16g.csv'); oldmap={(r['model'],r['geometry_id'],r['polarization'].lower(),int(float(r['wavelength_nm']))):r for r in old if r['variant']=='ensemble_raw'}; oldgeos=sorted({r['geometry_id'] for r in old}); idx={k:i for i,k in enumerate(keys(rows))}; lv=[];geomdelta=[]
 for m in MODELS:
 ...
 wcsv(OUT/'common_HF16_geometry_level_delta.csv',geomdelta);wcsv(OUT/'common_HF16_learning_value.csv',lv)
 # new4 heldout
 ...
 for s in sel:
  g=s['geometry_id'];
  for field,model in [('lf_eta_plus1','LF_only'),('calibrated_eta_plus1','LF_global_bias'),('ridge_eta_plus1','LF_ridge_residual'),('residual_mlp_eta_plus1','corrected_residual_mlp'),('cnn_eta_plus1','circular_cnn'),('direct_model_eta_plus1','direct_mlp')]:
 ...
 jwrite(OUT/'m8_training_run_manifest.json',{'status':'COMPLETE','preregistration_sha256':ph,'fit_started_after_preregistration':True,'fit_started_utc':now(),'fit_finished_utc':now(),'rows':440,'geometries':20,'paired_cases':40,'models':MODELS,'seeds':SEEDS,'epochs':EPOCHS,'outer_cv':'20-fold LOGO','solver_calls':0,'external_hf_calls':0,'sealed_target_reads':0,'u_x_scope':[0.0],'device':'torch+sklearn'})
 print(json.dumps({'status':'PASS','output':str(OUT),'rows':440,'geometries':20,'models':MODELS,'preregistration_sha256':ph,'solver_calls':0},indent=2))
