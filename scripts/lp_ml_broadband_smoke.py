...
PLAN=ROOT/'outputs/lp_ml_dataset_v1/plans/lp_ml_dataset_v1_round1_smoke_16_plan_v1.csv'
CONTRACT=ROOT/'outputs/lp_ml_dataset_v1/plans/lp_ml_dataset_v1_contract_v1.json'
ECONTRACT=ROOT/'outputs/lp_ml_dataset_v1/plans/lp_ml_dataset_v1_broadband_smoke_execution_contract_v1.json'
STAGE=ROOT/'outputs/lp_ml_dataset_v1/staging/lp_ml_dataset_v1_round1_smoke_v1'
TMP=Path(tempfile.gettempdir())/'lp_ml_dataset_v1_round1_smoke_runtime'
...
def build(fdtd,r,pol):
    ...
    fdtd.addpower(); fdtd.set('name','T'); fdtd.set('monitor type','2D Z-normal'); fdtd.set('x span',px); fdtd.set('y span',py); fdtd.set('z',1000*NM); fdtd.addprofile(); fdtd.set('name','field_monitor'); fdtd.set('monitor type','2D Z-normal'); fdtd.set('x span',px); fdtd.set('y span',py); fdtd.set('z',1000*NM)
...
def run_subrun(runtime,r,pol,entered,source_hash,contract_hash):
    ...
    try:
        ...
        entered.append(entry); write_csv(STAGE/'entered_accounting_v1.csv',entered); atomic_json(STAGE/'entered_accounting_v1.json',{'solver_entries':entered,'count':len(entered),'ceiling':32})
...
def build_gate(f,r,pol):
    ...
    checks={'FDTD_x_span':safe_get(f,'FDTD','x span'),'FDTD_y_span':safe_get(f,'FDTD','y span'),'source_start':safe_get(f,'source','wavelength start'),'source_stop':safe_get(f,'source','wavelength stop'),'source_pol':safe_get(f,'source','polarization angle'),'monitor_z':safe_get(f,'field_monitor','z'),'T_z':safe_get(f,'T','z'),'material_1':safe_get(f,'pillar_1','material'),'material_2':safe_get(f,'pillar_2','material')}
    ok=abs(float(checks['source_start'] or 0)-450e-9)<1e-15 and abs(float(checks['source_stop'] or 0)-454e-9)<1e-15 and abs(float(checks['monitor_z'] or 0)-1000e-9)<1e-15 and checks['material_1']==get_lumerical_material_name(MID) and checks['material_2']==get_lumerical_material_name(MID)
    return {'pass':bool(ok),'checks':checks,'objects':objects,'expected_wavelengths_nm':WLS,'input_polarization':pol,'observable':'coordinate_weighted_full_period_G0','normalization':'sqrt(T)/norm(weighted Ex,Ey)'}
...
def candidate_rows(r,x,y):
    ...
    for i,wl in enumerate(WLS):
        ...
        m=metrics(J); out.append({'candidate_id':r['candidate_id'],'category':r['category'],'wavelength_nm':wl,'geometry_hash_sha256':r['exact_geometry_hash_sha256'],'source_polarization_x_status':x['status'],'source_polarization_y_status':y['status'],'physics_origin':'PROSPECTIVE_LP_ML_ROUND1_SMOKE_FORMAL_WEIGHTED_G0','model_fill':'NONE','Jones_complete':True,**m,'phase_unwrapped_deg':None,'x_checkpoint_sha256':x.get('checkpoint_sha256'),'y_checkpoint_sha256':y.get('checkpoint_sha256')})
...
def main():
    ...
    if STAGE.exists(): raise SystemExit('HARD_GATE_STAGING_ALREADY_EXISTS')
    STAGE.mkdir(parents=True,exist_ok=False); TMP.mkdir(parents=True,exist_ok=True)
    protected={};
    ...
    if len(rows)!=16: raise SystemExit('HARD_GATE_SMOKE_PLAN_COUNT')
    atomic_json(STAGE/'preflight_environment_v1.json',{'hostname':socket.gethostname(),'cwd':str(ROOT),'branch':subprocess.check_output(['git','branch','--show-current'],cwd=ROOT,text=True).strip(),'head':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'source_contract_sha256':source_hash,'execution_contract_sha256':execution_hash,'protected_report_sha256_before':protected,'solver_authorized_subruns':32,'wavelengths_nm':WLS,'staging_identity':'LP_ML_DATASET_V1_ROUND1_SMOKE_V1'})
    runtime=load_runtime_config('configs/runtime.yaml'); lumapi=import_lumapi(runtime); runtime_lumapi=lumapi; entered=[]; sub=[]; cand=[]
    for idx,r in enumerate(rows,1):
        print(f'BEGIN {idx}/16 {r["candidate_id"]}',flush=True)
        ...
        cand.extend(cr); print(f'ACCEPT {idx}/16 {r["candidate_id"]}',flush=True)
    ...
    accepted=sum(q['status']=='ACCEPTED' for q in sub); entered_n=len(entered); outcome='LP_ML_PIPELINE_SMOKE_PASS_READY_FOR_ROUND1_PRODUCTION' if accepted==32 and len(cand)==144 else 'LP_ML_PIPELINE_SMOKE_PARTIAL_FIX_REQUIRED'
    qa={'outcome':outcome,'planned_geometries':16,'planned_subruns':32,'solver_entered':entered_n,'successful_accepted_subruns':accepted,'failed_subruns':len(sub)-accepted,'complete_geometries':len(cand)//9,'spectral_rows':len(cand),'duplicate_rows':len(cand)-len({(q['candidate_id'],q['wavelength_nm']) for q in cand}),'model_filled_rows':sum(q.get('model_fill')!='NONE' for q in cand),'protected_report_sha256_before':protected,'protected_report_sha256_after':{str(p):sha_bytes(p.read_bytes()) for p in [ROOT/'reports/lp_ml1a3_git_history_geometry_reconstruction.md',ROOT/'reports/stage11_4a20_legacy_fsp_object_inventory.md']},'no_d9_generated':True,'no_remaining_240_executed':True,'source_contract_sha256':source_hash,'execution_contract_sha256':execution_hash}
    atomic_json(STAGE/'quality_audit_v1.json',qa); atomic_json(STAGE/'dataset_manifest_v1.json',{'dataset':'LP_ML_DATASET_V1','schema_version':'LP_ML_DATASET_V1','outcome':outcome,'geometry_count':len(cand)//9,'spectral_row_count':len(cand),'solver_entries':entered_n,'physics_label':'FORMAL_WEIGHTED_G0_PROSPECTIVE_SMOKE','model_filled':False,'no_projector_pass_fail':True,'no_D9':True}); atomic_json(STAGE/'checksums_v1.json',{'files':{str(p.relative_to(STAGE)):sha_bytes(p.read_bytes()) for p in STAGE.rglob('*') if p.is_file() and p.name!='checksums_v1.json'}})
