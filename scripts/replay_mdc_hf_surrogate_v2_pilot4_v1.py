"""Fresh-process deterministic replay audit for frozen Pilot4 exports."""
...
from extract_mdc_hf_surrogate_v2_pilot4_labels_v1 import replay_digest
run = Path(sys.argv[1])
replay_id = int(sys.argv[2]) if len(sys.argv) > 2 else 1
out = run / f"pilot4_extraction_replay_{replay_id}.json"
d = replay_digest(run)
