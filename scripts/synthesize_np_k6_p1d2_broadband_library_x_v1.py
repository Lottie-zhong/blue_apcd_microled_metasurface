...
ALL_DIAMETERS=tuple(range(100,231,5))
DIAMETERS=tuple(d for d in ALL_DIAMETERS if d != 180)
LEGACY_MODE="legacy_26point_gap_v1"
COMPLETE_MODE="complete_27point_recovered_d180_v1"
...
def result_path(d:int)->Path:
    ...
    return matches[0]
def load(diameters: tuple[int,...]=DIAMETERS) -> tuple[list[int],dict[int,list[dict[str,Any]]],dict[str,str]]:
    rows={}; hashes={}
    for d in diameters:
...
def synthesize(out:Path=OUT, library_mode:str=LEGACY_MODE)->dict[str,Any]:
    if library_mode not in (LEGACY_MODE,COMPLETE_MODE): raise ValueError("unknown explicit library mode")
    ds,rows,hashes=load(DIAMETERS if library_mode==LEGACY_MODE else ALL_DIAMETERS); m=matrices(ds,rows); out.mkdir(parents=True,exist_ok=True)
    long=[]
    ...
    missing=[] if library_mode==COMPLETE_MODE else [180]
    manifest={"created_utc":now(),"offline_only":True,"lumapi_imported":False,"fdtd_run_called":False,"MPI_call_count":0,"library_mode":library_mode,"source_result_hashes":hashes,"included_diameters_nm":ds,"missing_diameters_nm":missing,"row_count":len(long),"wavelength_grid_nm":list(AXIS),"x_only":True,"interpolation_used":False}
    ...
    gap_pairs=adjacent(ds,m); gaps=[] if library_mode==COMPLETE_MODE else [x for x in gap_pairs["pairs"] if x["pair"]=="D175_to_D185"]
    if library_mode==LEGACY_MODE and len(gaps)!=1: raise RuntimeError("legacy gap contract requires exactly one D175-D185 gap")
    if library_mode==COMPLETE_MODE and gaps: raise RuntimeError("complete axis contract forbids missing-diameter gap")
    gap=gaps[0] if gaps else {"status":"not_applicable_complete_axis","crosses_missing_D180":False}
    ...
    trend={"diameters_nm":ds,"T_at_450_nm":[rows[d][5]["T"] for d in ds],"txx_amplitude_at_450_nm":[rows[d][5]["txx"]["amplitude"] for d in ds],"phase_at_450_nm_unwrapped_deg":[m["txx_unwrapped_phase_vs_diameter_deg"][i][5] for i in range(len(ds))],"missing_diameters_nm":missing,"axis_mode":"continuous_complete_27point_v1" if not missing else "gap_aware_26point_v1","interpolation_used":False}
    ...
    verification={"finite_data_gate":True,"library_row_count":len(long),"expected_row_count":len(ds)*11,"D180_included_gate":(180 in ds),"missing_diameters_nm":missing,"source_provenance_hashes_present":len(hashes)==len(ds),"x_only":True,"complete_27_point_library_claim":library_mode==COMPLETE_MODE,"interpolated_row_count":0}
    ...
    return manifest
def main()->int:
    p=argparse.ArgumentParser(); p.add_argument("--output",type=Path,default=OUT); p.add_argument("--library-mode",choices=(LEGACY_MODE,COMPLETE_MODE),default=LEGACY_MODE); a=p.parse_args(); print(json.dumps(synthesize(a.output,a.library_mode),indent=2)); return 0
