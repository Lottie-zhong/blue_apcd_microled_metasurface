...
def state_hash(sd):
    ...
    return h.hexdigest()
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--run-dir',required=True); ap.add_argument('--repo',required=True); ap.add_argument('--runner',required=True); args=ap.parse_args()
    run=Path(args.run_dir); repo=Path(args.repo); runner=Path(args.runner); commit=subprocess.check_output(['git','-C',str(repo),'rev-parse','HEAD'],text=True).strip(); membership=sha_file(run/'formal_membership_and_split_registry.json'); schema=sha_file(run/'v3_input_schema_contract.json')
    repaired=[]
