...
def main() -> None:
    ...
    boundary_counts = {"Explicit_boundary": sum(1 for g in geom_order if topo[g] == "Explicit" and meta[g].get("boundary_class") == "boundary"), "ZL1_boundary": sum(1 for g in geom_order if topo[g] == "ZL1" and meta[g].get("boundary_class") == "boundary"), "ZL2_boundary": sum(1 for g in geom_order if topo[g] == "ZL2" and meta[g].get("boundary_class") == "boundary")}
    sel_audit = {"status": "PASS", "selection_manifest": str(SEL), "selection_manifest_sha256": sha(SEL), "selected_geometry_count": 40, "selected_case_count": 240, "cases_per_geometry": 6, "topology_counts": {t: sum(1 for g in geom_order if topo[g] == t) for t in ("Explicit", "ZL1", "ZL2")}, "boundary_class_counts": boundary_counts, "boundary_class_quota_expected": 4, "v3_test40_overlap": 0, "selection_identity_only": True, "truth_values_not_used_for_selection": True}
