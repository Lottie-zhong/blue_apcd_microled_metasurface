...
ASSERT_NO_SOLVER_RUN = False
SLOT_MODULE = Path(r"D:\project\worktrees\blue_apcd_lp_global_h_manifold_v1\scripts")
if SLOT_MODULE.exists():
    sys.path.insert(0, str(SLOT_MODULE))
from apcd_global_fdtd_slot_v1 import GlobalSlotScheduler, SlotLease
...
EXPECTED_FDTD_RESOURCES = {"processes": "4", "threads": "1"}

def enforce_resource_gate(fdtd) -> dict:
    """Set and read back the frozen per-job resource contract before entry."""
    setter = getattr(fdtd, "setresource", None)
    if not callable(setter):
        raise RuntimeError("resource contract gate unavailable: setresource")
    for key, value in EXPECTED_FDTD_RESOURCES.items():
        setter("FDTD", 1, key, value)
    getter = getattr(fdtd, "getresource", None)
    if not callable(getter):
        raise RuntimeError("resource contract gate unavailable: getresource")
    readback = {}
    for key, expected in EXPECTED_FDTD_RESOURCES.items():
        value = getter("FDTD", 1, key)
        normalized = str(value).strip()
        readback[key] = normalized
        if normalized != expected:
            raise RuntimeError(f"resource contract mismatch {key}: expected {expected}, got {value!r}")
    return {"pass": True, "expected": dict(EXPECTED_FDTD_RESOURCES), "readback": readback}
...
def main() -> None:
    ...
    try:
        ...
        # Acquire the shared APCD global slot immediately before solver entry.
        scheduler = GlobalSlotScheduler()
        slot_lease = scheduler.acquire_wait("work/np-k6-mdc-v1", str(ROOT), case_id, case_id, pid=os.getpid(), metadata={"attempt_id": "attempt_001", "polarization": ledger.get("polarization"), "task_class": "NP_K6_M6_PRIMARY4_FORMAL_FDTD"}, timeout_s=21600.0, poll_s=15.0)
        ledger.update({"global_slot_id": slot_lease.slot_id, "global_slot_policy": "APCD_GLOBAL_FDTD_PARALLEL_POLICY_V1", "global_slot_acquired": True, "global_slot_acquired_timestamp_utc": now()})
        save_ledger(); event("global_slot_acquired", slot_id=slot_lease.slot_id)
        resource_gate = enforce_resource_gate(fd)
        ledger.update({"resource_contract": resource_gate})
        save_ledger(); event("resource_contract_verified", **resource_gate)
        ...
        save_ledger(); slot_lease.mark_solver_entered(ledger["solver_entered_timestamp_utc"]); event("solver_entered", run_invocation_count=1, slot_id=slot_lease.slot_id); heartbeat.set_state("solver_entered")
        if ASSERT_NO_SOLVER_RUN: raise RuntimeError("ASSERT_NO_SOLVER_RUN")
        fd.run()
        ...
        if slot_lease is not None:
            slot_lease.release("ENGINE_COMPLETED" if ledger.get("engine_completed") else "CONTROLLER_FAILED", ledger.get("engine_completed_timestamp_utc"))
