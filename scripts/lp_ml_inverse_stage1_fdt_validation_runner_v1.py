...
ECONTRACT=CONTRACT
ATTEMPT_ID='LP_ML_INVERSE_STAGE1_FDTD_VALIDATION_V1'
...
def extract_broadband(fdtd):
    ...
    if len(T)!=len(WLS): raise RuntimeError(f'T transmission length {len(T)} != {len(WLS)}')
    ...
    if ex.shape[2]!=len(WLS): raise RuntimeError(f'field frequency axis {ex.shape} != {len(WLS)}')
    out=[]
...
def run_subrun(runtime,r,pol,entered,source_hash,contract_hash):
    ...
    try:
        ...
        entered.append(entry); write_csv(STAGE/'entered_accounting_v1.csv',entered); atomic_json(STAGE/'entered_accounting_v1.json',{'solver_entries':entered,'count':len(entered),'ceiling':72})
        rec['solver_entered']=True; rec['failure_stage']='SOLVER'
        f.run(); rows,grid=extract_broadband(f); f.close(); f=None
        if len(rows)!=len(WLS): raise RuntimeError('incomplete 450 nm extraction')
        ...
        if re.get('status')!='ACCEPTED' or len(re.get('rows',[]))!=len(WLS): raise RuntimeError('checkpoint reload validation failed')
...
def build_gate(f,r,pol):
    ...
    checks={'FDTD_x_span':safe_get(f,'FDTD','x span'),'FDTD_y_span':safe_get(f,'FDTD','y span'),'background_material':safe_get(f,'FDTD','background material'),'background_index':safe_get(f,'FDTD','index'),'source_start':safe_get(f,'source','wavelength start'),'source_stop':safe_get(f,'source','wavelength stop'),'source_pol':safe_get(f,'source','polarization angle'),'monitor_z':safe_get(f,'field_monitor','z'),'T_z':safe_get(f,'T','z'),'material_1':safe_get(f,'pillar_1','material'),'material_2':safe_get(f,'pillar_2','material'),'J2_rotation_deg':safe_get(f,'pillar_2','rotation 1'),'T_override_global':safe_get(f,'T','override global monitor settings'),'T_use_wavelength_spacing':safe_get(f,'T','use wavelength spacing'),'T_frequency_points':safe_get(f,'T','frequency points'),'T_use_source_limits':safe_get(f,'T','use source limits'),'field_override_global':safe_get(f,'field_monitor','override global monitor settings'),'field_use_wavelength_spacing':safe_get(f,'field_monitor','use wavelength spacing'),'field_frequency_points':safe_get(f,'field_monitor','frequency points'),'field_use_source_limits':safe_get(f,'field_monitor','use source limits')}
    def eq(v,t):
    ...
    ok=eq(checks['source_start'],450e-9) and eq(checks['source_stop'],450e-9) and eq(checks['monitor_z'],1000e-9) and checks['material_1']==get_lumerical_material_name(MID) and checks['material_2']==get_lumerical_material_name(MID) and str(checks['background_material'])=='<Object defined dielectric>' and abs(float(checks['background_index'])-1.0)<1e-9 and abs(float(checks['J2_rotation_deg'])-fval(r,'Psi_deg'))<1e-8 and eq(checks['T_frequency_points'],1) and eq(checks['field_frequency_points'],1) and one(checks['T_override_global']) and one(checks['field_override_global']) and one(checks['T_use_wavelength_spacing']) and one(checks['field_use_wavelength_spacing']) and one(checks['T_use_source_limits']) and one(checks['field_use_source_limits'])
...
def candidate_rows(r,x,y):
    ...
    for i,wl in enumerate(WLS):
        ...
        m=metrics(J); out.append({'candidate_id':r['candidate_id'],'category':r.get('category'),'target_bin':r.get('target_bin'),'wavelength_nm':wl,'geometry_hash_sha256':r['exact_geometry_hash_sha256'],'source_polarization_x_status':x['status'],'source_polarization_y_status':y['status'],'physics_origin':'FORMAL_WEIGHTED_G0_PROSPECTIVE_ROUND3_TARGETED_ACTIVE_LEARNING','model_fill':'NONE','Jones_complete':True,**m,'phase_unwrapped_deg':None,'x_checkpoint_sha256':x.get('checkpoint_sha256'),'y_checkpoint_sha256':y.get('checkpoint_sha256')})
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--execute',action='store_true'); ap.add_argument('--preflight-only',action='store_true'); ap.add_argument('--resume',action='store_true'); args=ap.parse_args()
    if not args.execute and not args.preflight_only: raise SystemExit('explicit --execute or --preflight-only required')
    if args.execute and STAGE.exists() and not args.resume: raise SystemExit('HARD_GATE_NEW_ATTEMPT_STAGING_ALREADY_EXISTS')
    if args.execute and not STAGE.exists(): STAGE.mkdir(parents=True,exist_ok=False)
    TMP.mkdir(parents=True,exist_ok=True)
    protected={str(p):sha_bytes(p.read_bytes()) for p in [ROOT/'reports/lp_ml1a3_git_history_geometry_reconstruction.md',ROOT/'reports/stage11_4a20_legacy_fsp_object_inventory.md']}
    ...
    if len(rows)!=36 or any(r['candidate_id']=='LPML_R1_GLOBAL_SOBOL_054' for r in rows) or len({r['exact_geometry_hash_sha256'] for r in rows})!=36 or len({r['canonical_relative_geometry_hash_sha256'] for r in rows})!=36 or len({r['symmetry_equivalence_geometry_hash_sha256'] for r in rows})!=36: raise SystemExit('HARD_GATE_STAGE1_MANIFEST')
    env={'hostname':socket.gethostname(),'cwd':str(ROOT),'branch':subprocess.check_output(['git','branch','--show-current'],cwd=ROOT,text=True).strip(),'head':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'source_contract_sha256':source_hash,'execution_contract_sha256':execution_hash,'protected_report_sha256_before':protected,'solver_authorized_subruns':72,'wavelengths_nm':WLS,'attempt_id':ATTEMPT_ID,'staging_identity':'LP_ML_INVERSE_STAGE1_FDT_VALIDATION_V1','quarantined_candidate':'LPML_R1_GLOBAL_SOBOL_054','pre_retrain_prediction_status':'ROUND3_PRE_FDTD_SURROGATE_PREDICTION_FROZEN'}
    runtime=load_runtime_config('configs/runtime.yaml'); lumapi=import_lumapi(runtime); proxy=type('RuntimeProxy',(),{'lumapi':lumapi,'hide_gui':getattr(runtime,'hide_gui',True)})()
    if args.preflight_only:
        r=rows[0]; f=lumapi.FDTD(hide=getattr(runtime,'hide_gui',True)); fp=TMP/'preflight_sampling_gate.fsp'; setup=build(f,r,'x'); f.save(str(fp)); pre_hash=sha_bytes(fp.read_bytes()); f.close(); f=lumapi.FDTD(hide=getattr(runtime,'hide_gui',True)); f.load(str(fp)); gate=build_gate(f,r,'x'); f.close(); fp.unlink(missing_ok=True); env.update({'preflight_setup':setup,'preflight_reload_gate':gate,'solver_entered':False,'preflight_polarization':'x'}); atomic_json(ROOT/'outputs/lp_ml_dataset_v1/analysis/lp_ml_dataset_v1_round2_preflight_sampling_gate_v1.json',env); print(json.dumps(env,indent=2,default=str)); return
    atomic_json(STAGE/'preflight_environment_v1.json',env); entered=[]; sub=[]; cand=[]; failed_cases=[]; systemic=False
    if args.resume and (STAGE/'entered_accounting_v1.json').exists():
        entered=json.loads((STAGE/'entered_accounting_v1.json').read_text(encoding='utf-8')).get('solver_entries',[])
    def load_existing(cid, pol):
        q=STAGE/'subruns'/cid/pol/'run_result.json'
        chk=STAGE/'subruns'/cid/pol/'checkpoint.json'
        try:
            if chk.exists():
                c=json.loads(chk.read_text(encoding='utf-8'))
                if c.get('status')=='ACCEPTED' and len(c.get('rows',[]))==len(WLS):
                    return {'subrun_id':f'{cid}_{pol}','candidate_id':cid,'input_polarization':pol,'status':'ACCEPTED','solver_entered':True,'rows':c['rows'],'grid_audit':c.get('grid_audit'),'checkpoint_path':str(chk),'checkpoint_sha256':sha_bytes(chk.read_bytes()),'geometry_hash_sha256':c.get('geometry',{}).get('exact_geometry_hash_sha256'),'source_contract_sha256':c.get('source_contract_sha256'),'physical_contract_sha256':c.get('physical_contract_sha256'),'recovered_from_checkpoint':True}
            if not q.exists(): return None
            return json.loads(q.read_text(encoding='utf-8'))
        except Exception: return None
    for idx,r in enumerate(rows,1):
        results={}; print(f'BEGIN {idx}/36 {r["candidate_id"]}',flush=True)
        x=load_existing(r['candidate_id'],'x') if args.resume else None
        if x is None: x=run_subrun(proxy,r,'x',entered,source_hash,contract_hash)
        sub.append(x); results['x']=x
        if x['status']!='ACCEPTED':
            failed_cases.append(x); print(f'QUARANTINE_X {r["candidate_id"]}',flush=True); continue
        y=load_existing(r['candidate_id'],'y') if args.resume else None
        if y is None: y=run_subrun(proxy,r,'y',entered,source_hash,contract_hash)
        sub.append(y); results['y']=y
        if y['status']!='ACCEPTED':
            failed_cases.append(y); print(f'QUARANTINE_Y {r["candidate_id"]}',flush=True); continue
        cr=candidate_rows(r,x,y); cp=STAGE/'candidates'/r['candidate_id']; cp.mkdir(parents=True,exist_ok=True); atomic_json(cp/'candidate_checkpoint.json',{'candidate_id':r['candidate_id'],'geometry':dict(r),'rows':cr,'x_checkpoint':x['checkpoint_path'],'y_checkpoint':y['checkpoint_path'],'status':'ACCEPTED'}); cand.extend(cr); print(f'ACCEPT {idx}/36 {r["candidate_id"]}',flush=True)
    write_csv(STAGE/'subrun_records_v1.csv',[{k:v for k,v in q.items() if k!='rows'} for q in sub]); write_csv(STAGE/'candidate_wavelength_jones_v1.csv',cand); write_csv(STAGE/'geometry_records_v1.csv',[dict(r) for r in rows if r['candidate_id'] in {q['candidate_id'] for q in cand}]); accepted=sum(q['status']=='ACCEPTED' for q in sub); entered_n=len(entered)
    outcome='LP_ML_INVERSE_STAGE1_FDTD_PASS_READY_FOR_ASSEMBLY' if entered_n<=72 and not failed_cases and accepted==72 and len(cand)==36 else ('LP_ML_INVERSE_STAGE1_PARTIAL_DATA_PRESERVED' if entered_n<=72 else 'LP_ML_INVERSE_STAGE1_HARD_GATE')
    qa={'outcome':outcome,'planned_geometries':36,'planned_subruns':72,'solver_entered':entered_n,'successful_accepted_subruns':accepted,'failed_subruns':len(sub)-accepted,'quarantined_cases':[q['subrun_id'] for q in failed_cases],'complete_recovery_geometries':len(cand),'spectral_rows':len(cand),'duplicate_rows':len(cand)-len({(q['candidate_id'],q['wavelength_nm']) for q in cand}),'model_filled_rows':sum(q.get('model_fill')!='NONE' for q in cand),'protected_report_sha256_before':protected,'protected_report_sha256_after':{str(p):sha_bytes(p.read_bytes()) for p in [ROOT/'reports/lp_ml1a3_git_history_geometry_reconstruction.md',ROOT/'reports/stage11_4a20_legacy_fsp_object_inventory.md']},'no_054_generated':True,'no_accepted_rerun':True,'systemic_failure':systemic,'source_contract_sha256':source_hash,'execution_contract_sha256':execution_hash}
    atomic_json(STAGE/'quality_audit_v1.json',qa); atomic_json(STAGE/'dataset_manifest_v1.json',{'dataset':'LP_ML_INVERSE_STAGE1','schema_version':'LP_ML_INVERSE_STAGE1','outcome':outcome,'geometry_count':len(cand),'spectral_row_count':len(cand),'solver_entries':entered_n,'physics_label':'FORMAL_WEIGHTED_G0_PROSPECTIVE_INVERSE_STAGE1','model_filled':False,'no_D9':True,'pre_fdt_predictions_immutable':True}); atomic_json(STAGE/'failure_and_quarantine_v1.json',{'failed_cases':failed_cases,'policy':'isolated entered failures quarantined; no retry; untouched continuation completed'}); atomic_json(STAGE/'final_sentinel_v1.json',{'outcome':outcome,'entered':entered_n,'accepted':accepted,'ceiling':72,'quarantined_count':len(failed_cases)}); atomic_json(STAGE/'checksums_v1.json',{'files':{str(p.relative_to(STAGE)):sha_bytes(p.read_bytes()) for p in STAGE.rglob('*') if p.is_file() and p.name!='checksums_v1.json'}}); print(json.dumps(qa,indent=2,default=str),flush=True)
