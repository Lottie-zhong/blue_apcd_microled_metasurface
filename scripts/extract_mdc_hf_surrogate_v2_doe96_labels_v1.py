"""Extract frozen DOE96 raw joint profiles into case/geometry/NP views."""
...
def main(run):
 ...
 if len(cases)!=576 or not all(c.get('accepted') and c.get('solver_status')=='COMPLETE' for c in cases): raise RuntimeError('doe96_not_all_accepted')
 ...
 pd.DataFrame(geometry_rows).to_parquet(out/'doe96_geometry_labels_v1.parquet',index=False); pd.DataFrame(profile_rows).to_parquet(out/'doe96_geometry_profile_index_v1.parquet',index=False)
 dump(out/'doe96_geometry_label_manifest_v1.json',{'geometry_count':96,'case_count_consumed':576,'normalized_joint_profile_count':96,'auxiliary_label_fields':['peak_wavelength_nm','spectral_fwhm_nm','peak_angle_deg','angular_fwhm_deg','cone5','cone10','cone20'],'aggregation_contract':'fixed_v2_aggregation_contract_v1','parquet_path':str(out/'doe96_geometry_labels_v1.parquet')})
 dump(out/'doe96_aggregation_audit_v1.json',{'status':'PASS','geometry_count':96,'case_count':576,'per_geometry':aggregate_audit,'raw_before_normalization':True,'case_normalization_before_aggregation':False,'x_z_complex_field_interference':False,'all_integrals_close_to_one':all(abs(x['normalized_profile_integral']-1.0)<1e-9 for x in aggregate_audit)})
 pd.DataFrame(np_rows).to_parquet(out/'doe96_np_interface_view_v1.parquet',index=False); dump(out/'doe96_np_interface_contract_resolved.json',{'contract_id':'doe96_np_interface_contract_resolved_v1','input_fields':['wavelength_grid','angle_grid','channel/orientation convention','normalized joint MDC weight','relative total power','geometry_id/hash','profile SHA'],'fixture':'synthetic frozen NP eta response; no NP solver','grid_alignment':'exact same grid SHA','interpolation_policy':'none','extrapolation':'forbidden','weighted_integration':'integral W_MDC(lambda,theta)*eta_NP,+1 over lambda/theta','target_order':'+1','target_direction_deg':15.0,'low_reflection_caveat':'modular interface only in weak-reflection regime; full-wave validation required if NP alters MDC cavity','uses_joint_profile':True})
 dump(out/'doe96_np_interface_consumption_test.json',{'status':'PASS','geometry_count':96,'profile_rows':96,'np_order':'+1','grid_alignment_pass':True,'interpolation_used':False,'extrapolation_used':False,'joint_profile_consumed':True,'fwhm_only_input':False,'wavelength_angle_independence_assumed':False,'weighted_outputs_finite':all(np.isfinite(x['coupled_power_normalized']) for x in np_rows),'fixture_solver_calls':0})
 dump(out/'doe96_extraction_reproducibility_audit.json',{'status':'PENDING_REPLAY','required_fields':['case_index_sha256','joint_tensor_index_sha256','grid_sha256','geometry_profile_sha256','scalar_label_sha256','aggregation_membership_sha256','np_interface_view_sha256']})
 print(json.dumps({'status':'PASS','case_count':576,'geometry_count':96,'max_shape':sorted({tuple(a['shape']) for a in quality})[-1]},sort_keys=True))
