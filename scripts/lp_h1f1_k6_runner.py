...
MANIFEST_PATH = REPORT / "h1f1_candidate_manifest.json"
CASE_ROOT = ROOT / "outputs/lp_h1f1_k6_coupling_level0/cases"
ACCOUNTING_PATH = REPORT / "h1f1_solver_accounting.json"
...
def manifest():
    ...
    if m.get("status") != "FROZEN_READY_FOR_SETUP" or m.get("candidate_count") != 3:
    ...
    if len(m.get("candidates", [])) != 3 or any(not c.get("no_position_shift") for c in m["candidates"]):
...
def identity(candidate, pol):
    return {"case_uid": f"H1F1_{candidate['candidate_uid']}_{pol}", "candidate_uid": candidate["candidate_uid"], "candidate_hash": candidate["candidate_hash"], "polarization": pol, "manifest_freeze_sha256": manifest()["freeze_sha256"], "wavelength_grid_nm": GRID, "solver_runs_for_spectrum": 1}
...
def recover_entered(m):
    ...
    for candidate in m["candidates"]:
        for pol in POLS:
            case_uid = f"H1F1_{candidate['candidate_uid']}_{pol}"
...
def postprocess(m):
    ...
    for c in m["candidates"]:
        for pol in POLS:
            checkpoint = CASE_ROOT / f"H1F1_{c['candidate_uid']}_{pol}" / "attempt_001" / "checkpoint.json"
    ...
    write_csv(REPORT / "h1f1_order_resolved_fullwave.csv", rows)
    jrows = []
    for c in m["candidates"]:
    ...
    write_csv(REPORT / "h1f1_k6_order_jones.csv", jrows)
