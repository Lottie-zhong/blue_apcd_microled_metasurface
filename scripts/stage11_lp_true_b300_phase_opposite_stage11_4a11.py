...
RESULT_CSV = REPORT_DIR / "stage11_4a11_true_b300_phase_opposite_results.csv"
RANKING_CSV = REPORT_DIR / "stage11_4a11_true_b300_phase_opposite_ranking.csv"
SUMMARY_JSON = REPORT_DIR / "stage11_4a11_true_b300_phase_opposite_summary.json"
REPORT_MD = REPORT_DIR / "stage11_4a11_true_b300_phase_opposite_report.md"
NEXT_MD = REPORT_DIR / "stage11_4a11_true_b300_phase_opposite_recommended_next.md"
...
def validate_group() -> None:
    ...
    if not group or int(group["planned_cases"]) != 12:
        raise RuntimeError("A9 G2 must be exactly 12 cases")


G2_SOURCE_IDS = {
    "H500DIMER12A_008_B240_x_pair_swap_G90_O-32",
    "H500DIMER12A_002_B240_x_pair_swap_G85_O-28",
    "H500DIMER12A_003_B240_x_pair_swap_G95_O-28",
    "H500DIMER12A_006_B240_x_pair_swap_G80_O-28",
}
...
def source_rows() -> dict[str, dict[str, str]]:
    ...
    for row in data.get("cases", []):
        ...
        if cid in G2_SOURCE_IDS and cid not in out:
            out[cid] = dict(row)
    return out
...
def geometries() -> list[dict[str, str]]:
    src = source_rows()
    specs = [
        ("H500DIMER12A_008_B240_x_pair_swap_G90_O-32", "x_pair", "J2-J1", 90.0),
        ("H500DIMER12A_002_B240_x_pair_swap_G85_O-28", "x_pair", "J2-J1", 85.0),
        ("H500DIMER12A_003_B240_x_pair_swap_G95_O-28", "x_pair", "J2-J1", 95.0),
        ("H500DIMER12A_006_B240_x_pair_swap_G80_O-28", "x_pair", "J2-J1", 80.0),
    ]
    rows = []
...
def run_matrix() -> list[dict[str, str]]:
    ...
    for idx, base in enumerate(geometries(), start=1):
        for wl in WAVELENGTHS:
            ...
            cid = f"H600TRUEB300G2_{idx:03d}_{base['placement_type']}_{base['swap_order'].replace('-', '')}_G{gap}"
            case_id = f"S11_4A11_TRUE_B300_G2_{idx:03d}_WL{wl}"
            row = dict(base)
            row.update({
                "case_id": case_id,
                ...
                "notes": "Stage11-4A11 true B300 G1 phase-opposite only; no G3, coverage, H650, K6.",
            })
            rows.append(row)
    if len(rows) != 12:
        raise RuntimeError(f"Expected 12 A11 rows, got {len(rows)}")
    return rows
...
def write_reports(rows: list[dict[str, str]], counts: dict[str, int]) -> None:
    ...
    if strict or loose:
        ...
        next_step = "Run A9 G3 mini escape; G2 found no useful true B300."
    summary = {"stage": "Stage11-4A11", "group_id": GROUP_ID, **counts, "strict_candidates": len(strict), "loose_candidates": len(loose), "near_miss_candidates": len(near), "reassigned_candidates": len(reassigned), "best_candidate": best, "a8_b60_donor_reference": A8_B60_DONOR, "recommended_next": next_step, "no_g3": True, "no_coverage": True, "no_h650": True, "no_k6": True}
    SUMMARY_JSON.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    REPORT_MD.write_text("\n".join(["# Stage11-4A11 True B300 Different-Anchor Scout", "", "Scope: only A9 G2, 12 H600 true-B300 phase-opposite cases. No G2/G3, coverage, H650, K=6, H500 rescue.", "", f"planned = {counts['planned']}", f"run = {counts['run']}", f"failed = {counts['failed']}", f"missing = {counts['missing']}", f"strict = {len(strict)}", f"loose = {len(loose)}", f"near_miss = {len(near)}", f"reassigned = {len(reassigned)}", "", "## Best Candidate", "", "```json", json.dumps(best, indent=2), "```", "", "## A8 B60 Donor Reference", "", json.dumps(A8_B60_DONOR, indent=2), "", "## Recommendation", "", next_step, ""]), encoding="utf-8")
    NEXT_MD.write_text("# Stage11-4A11 Recommended Next\n\n" + next_step + "\n", encoding="utf-8")
