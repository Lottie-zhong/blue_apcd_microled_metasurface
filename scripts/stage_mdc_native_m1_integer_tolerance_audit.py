...
def postprocess_only():
    ...
    for cid in CASES:
        ...
        for rank,r in enumerate(rr[:10],1): candidates.append({'candidate_id':cid,'geometry':r['sequence_json'],'scan_mode':'design_basin','error_bound_nm':0,'spectral_peak_nm':r['spectral_peak_nm'],'spectral_FWHM_nm':r['spectral_FWHM_nm'],'T450':r['T450'],'max_transmission_angle_450_deg':r['max_transmission_angle_450_deg'],'strict_normal':str(r['strict_normal']).lower(),'near_normal':str(r['near_normal']).lower(),'angular_FWHM_450_deg':r['angular_FWHM_450_deg'],'spectral_target_pass':str(r['spectral_target_pass']).lower(),'angular_target_pass':str(r['angular_target_pass']).lower(),'combined_pass':str(r['combined_pass']).lower(),'robustness_score':0.0,'selection_reason':f'best integer design-basin ordering; rank {rank}'})
