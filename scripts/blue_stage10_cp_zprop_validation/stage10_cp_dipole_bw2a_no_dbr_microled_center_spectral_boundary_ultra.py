...
WAVELENGTHS = [420.0, 422.0, 424.0, 426.0, 474.0, 476.0, 478.0, 480.0]
EXISTING_COMBINED = ROOT / "outputs" / "stage10_cp_dipole_bw2a_no_dbr_microled_center_spectral_boundary_wide" / "stage10_cp_dipole_bw2a_center_spectral_boundary_combined_summary.csv"
OUT_DIR = ROOT / "outputs" / "stage10_cp_dipole_bw2a_no_dbr_microled_center_spectral_boundary_ultra"
RUNTIME_DIR = Path(r"D:\project\bw2a_boundary_ultra_rt")
...
def planned_cases_16() -> list[dict[str, Any]]:
...
def fix_base_summary() -> None:
    ...
    data["stage"] = "Stage10-CP-DIPOLE-BW2A-CENTER-SPECTRAL-BOUNDARY-ULTRA"
    data["new_fdtd_cases_requested"] = 16
...
def configure_base() -> None:
    ...
    base.CANDIDATES = {
        CANDIDATE_ID: {
            "role": "primary robust CP candidate; no-DBR center spectral boundary one more outward bracket scout",
            "J1_rotation_deg": -9.0,
    ...
    base.planned_cases = planned_cases_16
...
def combined_rows() -> list[dict[str, Any]]:
    ...
    for row in read_csv(EXISTING_COMBINED):
        if row.get("candidate_id") == CANDIDATE_ID and row.get("source_position_label") == "center" and float(row.get("wavelength_nm", "nan")) in [428.0, 430.0, 432.0, 434.0, 436.0, 438.0, 440.0, 442.0, 444.0, 446.0, 447.0, 448.0, 450.0, 453.0, 454.0, 455.0, 456.0, 458.0, 460.0, 462.0, 464.0, 466.0, 468.0, 470.0, 472.0]:
            row = dict(row)
    ...
    for row in new_rows:
        row["data_source"] = "new_boundary_ultra"
...
def write_boundary_report(rows: list[dict[str, Any]]) -> None:
    ...
    report = [
        "# Stage10 CP BW2A PSI99 Center Spectral Boundary Ultra Scout\n",
        "- Scope: PSI99 only, no-DBR ordinary MicroLED center dipole only.",
        "- New wavelengths: 420, 422, 424, 426, 474, 476, 478, 480 nm.",
    ...
    summary = {
        "stage": "Stage10-CP-BW2A-PSI99-CENTER-SPECTRAL-BOUNDARY-ULTRA",
        "candidate_id": CANDIDATE_ID,
        "new_wavelengths_nm": WAVELENGTHS,
        "reused_wavelengths_nm": [428.0, 430.0, 432.0, 434.0, 436.0, 438.0, 440.0, 442.0, 444.0, 446.0, 447.0, 448.0, 450.0, 453.0, 454.0, 455.0, 456.0, 458.0, 460.0, 462.0, 464.0, 466.0, 468.0, 470.0, 472.0],
        ...
        "recommend_lower_extension": bool(blue_boundary is not None and blue_boundary <= 420.0),
        "recommend_upper_extension": bool(red_boundary is not None and red_boundary >= 480.0),
    }
