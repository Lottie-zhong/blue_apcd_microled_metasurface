...
WAVELENGTHS = [428.0, 430.0, 432.0, 434.0, 466.0, 468.0, 470.0, 472.0]
EXISTING_COMBINED = ROOT / "outputs" / "stage10_cp_dipole_bw2a_no_dbr_microled_center_spectral_boundary_extend" / "stage10_cp_dipole_bw2a_center_spectral_boundary_combined_summary.csv"
OUT_DIR = ROOT / "outputs" / "stage10_cp_dipole_bw2a_no_dbr_microled_center_spectral_boundary_wide"
RUNTIME_DIR = Path(r"D:\project\bw2a_boundary_wide_rt")
...
def fix_base_summary() -> None:
    ...
    data["stage"] = "Stage10-CP-DIPOLE-BW2A-CENTER-SPECTRAL-BOUNDARY-WIDE"
...
def configure_base() -> None:
    ...
    base.CANDIDATES = {
        CANDIDATE_ID: {
            "role": "primary robust CP candidate; no-DBR center spectral boundary wider outward scout",
            "J1_rotation_deg": -9.0,
...
def combined_rows() -> list[dict[str, Any]]:
    ...
    for row in read_csv(EXISTING_COMBINED):
        if row.get("candidate_id") == CANDIDATE_ID and row.get("source_position_label") == "center" and float(row.get("wavelength_nm", "nan")) in [436.0, 438.0, 440.0, 442.0, 444.0, 446.0, 447.0, 448.0, 450.0, 453.0, 454.0, 455.0, 456.0, 458.0, 460.0, 462.0, 464.0]:
            row = dict(row)
    ...
    for row in new_rows:
        row["data_source"] = "new_boundary_wide"
...
def write_boundary_report(rows: list[dict[str, Any]]) -> None:
    ...
    report = [
        "# Stage10 CP BW2A PSI99 Center Spectral Boundary Wide Scout\n",
        "- Scope: PSI99 only, no-DBR ordinary MicroLED center dipole only.",
        "- New wavelengths: 428, 430, 432, 434, 466, 468, 470, 472 nm.",
    ...
    summary = {
        "stage": "Stage10-CP-BW2A-PSI99-CENTER-SPECTRAL-BOUNDARY-WIDE",
        "candidate_id": CANDIDATE_ID,
        "new_wavelengths_nm": WAVELENGTHS,
        "reused_wavelengths_nm": [436.0, 438.0, 440.0, 442.0, 444.0, 446.0, 447.0, 448.0, 450.0, 453.0, 454.0, 455.0, 456.0, 458.0, 460.0, 462.0, 464.0],
        ...
        "recommend_lower_extension": bool(blue_boundary is not None and blue_boundary <= 428.0),
        "recommend_upper_extension": bool(red_boundary is not None and red_boundary >= 472.0),
    }
