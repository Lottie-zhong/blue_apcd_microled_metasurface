#!/usr/bin/env python3
"""R2-4F1 tri-point x-dipole FDTD guard for F0_0781 only.
...
OUT = ROOT / "outputs" / "r2_4f1_f0_0781_tri_point_xdipole_fdtd_guard"
SOLVE_DIR = OUT / "runtime_solve_fsp"
LUMAPI_DIR = Path(r"N:\Program Files\ANSYS Inc\v251\Lumerical\api\python")

STAGE = "R2-4F1 F0_0781 tri-point x-dipole 453 nm FDTD guard"
CANDIDATE_ID = "F0_0781"
FAMILY_ID = "top_MDC_angular_filter"
WAVELENGTH_NM = 453.0
X_POSITIONS_UM = [-0.7, 0.0, 0.7]
EXPECTED_CASE_IDS = [
    "F0_0781_xm0p7_xdipole_453",
    "F0_0781_x0p0_xdipole_453",
    "F0_0781_xp0p7_xdipole_453",
]
...
def make_layers() -> dict[str, list[dict[str, object]]]:
    ...
    idx = 0
    for _ in range(MDC_M):
        top.append({"layer_index": idx, "material": "SiO2", "thickness_nm": MDC_SIO2_NM}); idx += 1
        top.append({"layer_index": idx, "material": "TiO2", "thickness_nm": MDC_TIO2_NM}); idx += 1
...
def build_case(fdtd: Any, x_um: float, stacks: dict[str, list[dict[str, object]]]) -> dict[str, object]:
    ...
    case_id = f"F0_0781_{suffix}_xdipole_453"
...
def main() -> int:
    ...
    for x in X_POSITIONS_UM:
        case_manifest.append({"case_id": f"F0_0781_{case_suffix(x)}_xdipole_453", "source_x_um": x, "candidate_id": CANDIDATE_ID, "dipole_orientation": "x", "wavelength_nm": WAVELENGTH_NM})
    ...
    write_csv(OUT / "r2_4f1_case_manifest.csv", case_manifest)
    ...
    write_csv(OUT / "r2_4f1_case_results.csv", rows)
    ...
    if len(ok_rows) == 3 and len(cuts) == 3:
        ...
        write_csv(OUT / "r2_4f1_angle_cut_data.csv", angle_rows)
    else:
    ...
    write_csv(OUT / "r2_4f1_tri_point_incoherent_average.csv", avg_rows)
    ...
    write_json(OUT / "r2_4f1_manifest.json", manifest)

    summary = [
        "# R2-4F1 F0_0781 Tri-Point X-Dipole FDTD Guard",
        "",
        f"Completed ok cases: {len(ok_rows)} / 3.",
        f"Verdict: **{verdict}**.",
        "",
        "Scope was restricted to F0_0781, x-dipole only, 453 nm only, x = [-0.7, 0, +0.7] um.",
    ...
    write_text(OUT / "r2_4f1_summary.md", "\n".join(summary))

    diag = ["# R2-4F1 Source Position Stability Diagnosis", "", f"Verdict: {verdict}.", ""]
    if avg_rows:
        ar = avg_rows[0]
    ...
    write_text(OUT / "r2_4f1_source_position_stability_diagnosis.md", "\n".join(diag))

    verdict_md = ["# R2-4F1 Pass/Fail Verdict", "", f"Verdict: **{verdict}**.", "", "Rules applied:", "- hard fail if any case status != ok", "- hard fail if tri-point average peak_abs_angle > 8 deg", "- hard fail if tri-point average normal/offaxis <= 1", "- hard fail if any source revives a dominant 30-40, 45-55, or broad 40-60 deg lobe", "", f"Reasons: {avg_rows[0].get('verdict_reasons','missing') if avg_rows else 'missing'}"]
    write_text(OUT / "r2_4f1_pass_fail_verdict.md", "\n".join(verdict_md))

    if verdict == "pass":
        next_step = "F0_0781 passes tri-point guard. Next allowed step is a 5-point x-line x-dipole 453 nm plan only; do not execute it in F1."
    else:
        next_step = "F0_0781 fails or did not complete tri-point guard. Stop F0_0781; do not auto-run F0_0204, 5-point, 9-point, y/z dipole, or broadband FDTD."
    write_text(OUT / "r2_4f1_next_step_plan.md", "# R2-4F1 Next Step Plan\n\n" + next_step)
