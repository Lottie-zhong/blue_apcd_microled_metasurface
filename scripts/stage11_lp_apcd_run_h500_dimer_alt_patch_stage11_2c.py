...
PLAN=OUT_DIR/'h500_dimer_alt_patch_plan_stage11_2c.csv'; RESULT=OUT_DIR/'h500_dimer_alt_patch_fdtd_results_stage11_2c.csv'; SUMMARY=OUT_DIR/'h500_dimer_alt_patch_fdtd_summary_stage11_2c.md'; FDTD_DIR=OUT_DIR/'fdtd_h500_dimer_alt_patch'
def load_runner():
...
def write_summary(rows,dry=False):
    ...
    lines=['# Stage11-2C H500 Dimer Alt Patch FDTD Summary','',f"mode = {'dry_run_no_lumerical' if dry else 'real_h500_dimer_alt_patch_fdtd'}",f'result_count = {len(rows)}',f'success = {len(ok)}',f'failed = {len(failed)}','skipped = 0','','Only H500 dimer x/y normal-incidence FDTD. No K=6, no H600/H700.']
    SUMMARY.write_text('\n'.join(lines)+'\n',encoding='utf-8')
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--plan',default=str(PLAN)); ap.add_argument('--runtime',default='configs/runtime.yaml'); ap.add_argument('--max-cases',type=int,default=48); ap.add_argument('--dry-run',action='store_true'); args=ap.parse_args()
    ...
    print(f'selected_legal_alt_patch_count={len(rows)}')
    for r in rows[:min(12,len(rows))]: print(f"case={r['dimer_case_id']} target={r['bin_deg']} placement={r['placement_type']} swap={r.get('swap_order','')} gap={r.get('gap_nm','')} x/y")
    if args.dry_run:
    ...
    done={r.get('dimer_case_id') for r in existing if r.get('fdtd_status')=='ok'}
    results=list(existing)
    runtime=mod.load_runtime_config(args.runtime); lumapi=mod.import_lumapi(runtime)
    for r in rows:
        cid=r.get('dimer_case_id')
        if cid in done:
