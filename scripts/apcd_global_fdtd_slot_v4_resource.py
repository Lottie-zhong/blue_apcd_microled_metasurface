...
RESOURCE_POLICY_ID = "APCD_GLOBAL_FDTD_PRODUCTION_RESOURCE_POLICY_V4"
PROCESSES_PER_JOB = 12
THREADS_PER_JOB = 1
...
class GlobalSlotScheduler:
    ...
    def acquire(self,branch,worktree,task_id,case_uid,pid=None,metadata=None):
        ...
        if int(metadata.get("processes", PROCESSES_PER_JOB)) != PROCESSES_PER_JOB or int(metadata.get("threads", THREADS_PER_JOB)) != THREADS_PER_JOB:
            raise SlotError("RESOURCE_CONTRACT_MISMATCH_EXPECTED_12_MPI_1_THREAD")
        with registry_lock(self.registry_path):
