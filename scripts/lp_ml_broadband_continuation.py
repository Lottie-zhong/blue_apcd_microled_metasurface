...
PLAN=ROOT/'outputs/lp_ml_dataset_v1/plans/lp_ml_dataset_v1_round1_continuation_388_plan_v1.csv'
CONTRACT=ROOT/'outputs/lp_ml_dataset_v1/plans/lp_ml_dataset_v1_contract_v1.json'
ECONTRACT=ROOT/'outputs/lp_ml_dataset_v1/plans/lp_ml_dataset_v1_round1_continuation_execution_contract_v1.json'
ATTEMPT_ID='LP_ML_ROUND1_CONTINUATION_ATTEMPT1_V1'
STAGE=ROOT/'outputs/lp_ml_dataset_v1/staging/lp_ml_dataset_v1_round1_continuation_attempt1_v1'
TMP=Path(tempfile.gettempdir())/'lp_ml_dataset_v1_round1_continuation_attempt1_runtime'
...
def run_subrun(runtime,r,pol,entered,source_hash,contract_hash):
    ...
    try:
        ...
        entered.append(entry); write_csv(STAGE/'entered_accounting_v1.csv',entered); atomic_json(STAGE/'entered_accounting_v1.json',{'solver_entries':entered,'count':len(entered),'ceiling':388})
...
def candidate_rows(r,x,y):
    ...
    for i,wl in enumerate(WLS):
        ...
        m=metrics(J); out.append({'candidate_id':r['candidate_id'],'category':r['category'],'wavelength_nm':wl,'geometry_hash_sha256':r['exact_geometry_hash_sha256'],'source_polarization_x_status':x['status'],'source_polarization_y_status':y['status'],'physics_origin':'PROSPECTIVE_LP_ML_ROUND1_RECOVERY_FORMAL_WEIGHTED_G0','model_fill':'NONE','Jones_complete':True,**m,'phase_unwrapped_deg':None,'x_checkpoint_sha256':x.get('checkpoint_sha256'),'y_checkpoint_sha256':y.get('checkpoint_sha256')})
...
def main():
    ...
    if len(rows)!=194 or any(r['run_polarizations']!='x,y' for r in rows) or any(r['candidate_id']=='LPML_R1_GLOBAL_SOBOL_054' for r in rows): raise SystemExit('HARD_GATE_CONTINUATION_MANIFEST')
    env={'hostname':socket.gethostname(),'cwd':str(ROOT),'branch':subprocess.check_output(['git','branch','--show-current'],cwd=ROOT,text=True).strip(),'head':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'source_contract_sha256':source_hash,'execution_contract_sha256':execution_hash,'protected_report_sha256_before':protected,'solver_authorized_subruns':388,'wavelengths_nm':WLS,'attempt_id':ATTEMPT_ID,'staging_identity':'LP_ML_DATASET_V1_ROUND1_CONTINUATION_ATTEMPT1_V1','quarantined_candidate':'LPML_R1_GLOBAL_SOBOL_054'}
    ...
    outcome='LP_ML_ROUND1_CONTINUATION_PASS_READY_FOR_ASSEMBLY' if entered_n<=388 and not failed_cases and accepted==388 and len(cand)==1746 else ('LP_ML_ROUND1_CONTINUATION_ISOLATED_FAILURES_DATA_PRESERVED' if entered_n<=388 else 'LP_ML_ROUND1_CONTINUATION_HARD_GATE')
    qa={'outcome':outcome,'planned_geometries':194,'planned_subruns':388,'solver_entered':entered_n,'successful_accepted_subruns':accepted,'failed_subruns':len(sub)-accepted,'quarantined_cases':[q['subrun_id'] for q in failed_cases],'complete_recovery_geometries':len(cand)//9,'spectral_rows':len(cand),'duplicate_rows':len(cand)-len({(q['candidate_id'],q['wavelength_nm']) for q in cand}),'model_filled_rows':sum(q.get('model_fill')!='NONE' for q in cand),'protected_report_sha256_before':protected,'protected_report_sha256_after':{str(p):sha_bytes(p.read_bytes()) for p in [ROOT/'reports/lp_ml1a3_git_history_geometry_reconstruction.md',ROOT/'reports/stage11_4a20_legacy_fsp_object_inventory.md']},'no_054_generated':True,'no_accepted_rerun':True,'systemic_failure':systemic,'source_contract_sha256':source_hash,'execution_contract_sha256':execution_hash}
    atomic_json(STAGE/'quality_audit_v1.json',qa); atomic_json(STAGE/'dataset_manifest_v1.json',{'dataset':'LP_ML_DATASET_V1','schema_version':'LP_ML_DATASET_V1','outcome':outcome,'geometry_count':len(cand)//9,'spectral_row_count':len(cand),'solver_entries':entered_n,'physics_label':'FORMAL_WEIGHTED_G0_PROSPECTIVE_CONTINUATION','model_filled':False,'no_D9':True}); atomic_json(STAGE/'failure_and_quarantine_v1.json',{'failed_cases':failed_cases,'policy':'isolated entered failures quarantined; no retry; untouched continuation completed'}); atomic_json(STAGE/'final_sentinel_v1.json',{'outcome':outcome,'entered':entered_n,'accepted':accepted,'ceiling':388,'quarantined_count':len(failed_cases)}); atomic_json(STAGE/'checksums_v1.json',{'files':{str(p.relative_to(STAGE)):sha_bytes(p.read_bytes()) for p in STAGE.rglob('*') if p.is_file() and p.name!='checksums_v1.json'}}); print(json.dumps(qa,indent=2,default=str),flush=True)
