"""Authorized DOE96 joint wavelength-angle profile runner.

Uses the frozen 576-case matrix, the validated Native-M1 builder primitives and
fresh-load/save/run/extract lifecycle. The first matrix row is the upgrade smoke
case and is also a formal case; no case is retried automatically.
"""
...
CASE_MATRIX=ROOT/'contracts/mdc_hf_surrogate_v2/fixed_v2_initial_doe96_case_matrix.csv'
CANDIDATES=ROOT/'contracts/mdc_hf_surrogate_v2/fixed_v2_initial_doe96_candidate_manifest.json'
READINESS=ROOT/'contracts/mdc_hf_surrogate_v2/fixed_v2_database_readiness_manifest.json'
MATERIAL_CONFIG=ROOT/'configs/material_reference_apcd_blue.yaml'
RUNTIME_ROOT=ROOT/'runtime/mdc_hf_surrogate_v2_doe96_joint_profile_v1'
...
def read_matrix():
    ...
    if len(rows)!=576: raise RuntimeError('doe96_case_count_drift')
    if len({r['geometry_hash'] for r in rows})!=96 or len({r['case_hash'] for r in rows})!=576: raise RuntimeError('doe96_hash_count_drift')
    ...
    expected={'top':'-171.5','centroid':'-276.0','bottom':'-380.5'}
    if any(r['source_position_nm']!=expected[r['source_position']] for r in rows): raise RuntimeError('doe96_source_nm_drift')
...
def candidate_map():
    c=load(CANDIDATES)
    if c.get('candidate_count')!=96 or c.get('case_count')!=576: raise RuntimeError('doe96_candidate_manifest_drift')
    return {x['geometry_hash']:x for x in c['candidates']}
...
def realization(rec):
    ...
    """
    n=int(rec['layer_count']); total=int(rec['total_thickness_nm']); defect=int(rec['defect_thickness_nm'])
    if n<3 or defect<=0 or defect>=total: raise RuntimeError('invalid_candidate_geometry_fields')
    idx=n//2; remaining=total-defect; base,extra=divmod(remaining,n-1)
    thickness=[]
    for j in range(n):
        if j==idx: thickness.append(defect)
        else:
            q=base+(1 if (j if j<idx else j-1)<extra else 0); thickness.append(q)
    seq=[]
    for j,t in enumerate(thickness):
        token='H' if j%2==0 else 'L'
        if j==idx: token='H'
        seq.append((token,float(t)))
...
def extract_case(case,run_root,state,smoke=False):
    ...
    try:
        ...
        with (run_root/'doe96_case_attempt_ledger.jsonl').open('a',encoding='utf-8') as lf: lf.write(json.dumps(ledger,sort_keys=True)+'\n')
...
def initialize(run_root):
    ...
    if len(cand)!=96: raise RuntimeError('candidate_count_drift')
    auth={'solver_authorized':True,'authorized_tier':'DOE96','authorized_geometry_count':96,'authorized_case_count':576,'authorization_source':'EXPLICIT_USER_APPROVAL','authorization_date':'2026-08-03','DOE96_authorized':True,'external_test40_authorized':False,'model_training_authorized':False,'profile_compression_fit_authorized':False,'active_learning_authorized':False,'HF15_formal_reads_authorized':False,'sealed_test_authorized':False}
    dump(run_root/'doe96_solver_authorization.json',auth)
    ...
    cases=[]
    for r in rows: cases.append(dict(r)|{'case_id':f"{r['geometry_hash'][:16]}__{r['source_position']}__{r['dipole_orientation']}",'status':'PENDING','attempt_count':0,'accepted':False})
    state={'run_id':run_root.name,'created_at':now(),'cases':{r['case_hash']:r for r in cases},'safety_counters':{'solver_calls':0,'fdtd_lumerical_calls':0,'recovery_solver_calls':0,'TMM_calls':0,'RCWA_calls':0,'model_fits':0,'optimizer_backward':0,'HF15_formal_reads':0,'HF15_diagnostics_reads':0,'sealed_test_reads':0,'DOE96_solver_calls':0,'NP_solver_calls':0,'test40_reads':0,'active_learning_acquisitions':0,'compression_fits':0},'authorization':auth,'geometry_realizations':[realization(x) for x in cand.values()]}
    dump(run_root/'state.json',state); (run_root/'doe96_case_attempt_ledger.jsonl').write_text('',encoding='utf-8')
    return state

def smoke_audit(run_root,state,case_result):
    a={'status':'PASS','smoke_case_id':case_result['case_id'],'solver_completed':case_result['solver_status']=='COMPLETE','post_fsp_exists':Path(case_result['fsp_path']).exists(),'fresh_load_status':case_result['fresh_load_status'],'joint_tensor_exists':Path(case_result['raw_npz_path']).exists(),'tensor_shape':case_result['joint_shape'],'wavelength_bins_gt_1':case_result['joint_shape'][0]>1,'angle_bins_gt_1':case_result['joint_shape'][1]>1,'finite_ratio':1.0-case_result['joint_nonfinite_ratio'],'raw_negative_count':case_result['joint_negative_count'],'spectral_marginal_recovered_from_joint':True,'angular_marginal_recovered_from_joint':True,'normalization_before_aggregation':case_result['normalization_before_aggregation'],'geometry_hash_consistent':True,'case_hash_consistent':True,'solver_calls_added':1,'unique_case_count_added':1,'upgrade_contract_sha256':sha(run_root/'joint_profile_export_contract_resolved.json')}
    ...
    dump(run_root/'doe96_upgrade_smoke_audit.json',a); return a

def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--output-root',required=True); ap.add_argument('--init',action='store_true'); ap.add_argument('--smoke',action='store_true'); ap.add_argument('--continue-run',action='store_true'); ap.add_argument('--batch',type=int,default=0); args=ap.parse_args(); run_root=Path(args.output_root).resolve()
    ...
    if args.continue_run:
        smoke=load(run_root/'doe96_upgrade_smoke_audit.json')
        if smoke.get('status')!='PASS': raise RuntimeError('smoke_gate_not_passed')
        ...
        if args.batch:
            if args.batch < 1 or args.batch > 6: raise RuntimeError('batch_index_out_of_range')
        ...
        dump(run_root/'doe96_batch_gate_report.json',{'batch_index':args.batch or 0,'geometry_count':len(selected),'expected_case_count':len(selected_rows),'completed_case_count':sum(state['cases'][h].get('solver_status')=='COMPLETE' for h in selected_hashes),'accepted_case_count':sum(bool(state['cases'][h].get('accepted')) for h in selected_hashes),'geometry_ids':sorted(selected),'status':'PASS' if all(state['cases'][h].get('accepted') for h in selected_hashes) else 'HARD_GATE_DOE96_BATCH_INCOMPLETE'})
        dump(run_root/'doe96_solver_run_manifest.json',{'run_id':run_root.name,'authorized_geometry_count':96,'authorized_unique_physical_cases':576,'completed_unique_physical_cases':sum(c.get('solver_status')=='COMPLETE' for c in state['cases'].values()),'accepted_cases':sum(bool(c.get('accepted')) for c in state['cases'].values()),'rejected_unresolved_cases':sum(not bool(c.get('accepted')) for c in state['cases'].values()),'total_solver_calls':state['safety_counters']['solver_calls'],'recovery_solver_calls':state['safety_counters']['recovery_solver_calls'],'DOE96_solver_calls':state['safety_counters']['DOE96_solver_calls'],'status':'DOE96_SOLVER_RUN_COMPLETE' if all(c.get('accepted') for c in state['cases'].values()) else 'DOE96_SOLVER_RUN_IN_PROGRESS','safety_counters':state['safety_counters']})
        print(json.dumps(load(run_root/'doe96_solver_run_manifest.json'),sort_keys=True))
