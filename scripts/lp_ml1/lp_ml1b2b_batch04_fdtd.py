...
OUT = ROOT / "outputs" / "lp_ml1b2b_36case_pilot" / "batch_04"
TMP_FDTD = OUT / "fdtd_tmp"
RESULTS = OUT / "lp_ml1b2b_batch04_results.csv"
FAILURES = OUT / "lp_ml1b2b_batch04_failure_log.csv"
RUNTIME = OUT / "lp_ml1b2b_batch04_runtime_manifest.csv"
SUMMARY = OUT / "lp_ml1b2b_batch04_summary.json"
RANKING = OUT / "lp_ml1b2b_batch04_candidate_ranking.csv"
REPORT = ROOT / "reports" / "lp_ml1b2b_batch04_execution_report.md"
...
ANOMALIES = OUT / "lp_ml1b2b_batch04_anomaly_flags.csv"
EPS = 1e-12
...
def selected_rows() -> list[dict[str, str]]:
    ...
    if len(ids) > 6:
        raise ValueError(f"Batch-04 too large: {len(ids)} candidates")
...
def main() -> None:
    ...
    report = ["# LP-ML1B2B batch-04 execution report", "", f"Batch: {BATCH_ID}", "", "## Candidate table", "", "| candidate_id | target_bin | sampling_group | H_nm | wavelengths | polarizations | subruns |", "|---|---:|---|---:|---:|---:|---:|" ]
    for row in rows:
