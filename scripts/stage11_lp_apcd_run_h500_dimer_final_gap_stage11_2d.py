...
PLAN=OUT_DIR/'h500_dimer_final_gap_patch_plan_stage11_2d.csv'; RESULT=OUT_DIR/'h500_dimer_final_gap_fdtd_results_stage11_2d.csv'; SUMMARY=OUT_DIR/'h500_dimer_final_gap_fdtd_summary_stage11_2d.md'; FDTD_DIR=OUT_DIR/'fdtd_h500_dimer_final_gap'
def load_runner():
...
def write_summary(rows,dry=False):
    ...
    lines=['# Stage11-2D H500 Final Gap FDTD Summary','',f"mode = {'dry_run_no_lumerical' if dry else 'real_h500_final_gap_fdtd'}",f'result_count = {len(rows)}',f'success = {len(ok)}',f'failed = {len(failed)}','skipped = 0','','Only H500 dimer x/y normal-incidence FDTD. No K=6, no H600/H700.']
    SUMMARY.write_text('\n'.join(lines)+'\n',encoding='utf-8')
def main():
    ...
    print(f'selected_legal_final_gap_count={len(rows)}')
    ...
    done={r.get('dimer_case_id') for r in existing if r.get('fdtd_status')=='ok'}; results=list(existing)
