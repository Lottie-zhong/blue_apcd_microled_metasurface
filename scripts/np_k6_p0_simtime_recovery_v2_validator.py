...
def main():
 c=j('classification.json'); b=j('closure_audit.json'); l=j('entered_ledger.json'); r=j('runtime_execution_audit.json'); p=j('post_fsp_checksum.json'); q=j('provenance_audit.json'); d=j('data_gate.json')
 ...
 assert all(math.isfinite(float(x['T_total'])) and math.isfinite(float(x['R_total'])) for x in rows)
