...
PLAN=ROOT/'outputs/lp_ml_dataset_v1/plans/lp_ml_dataset_v1_round3_64_candidate_plan_v1.csv'
CONTRACT=ROOT/'outputs/lp_ml_dataset_v1/plans/lp_ml_dataset_v1_round3_execution_contract_v1.json'
ECONTRACT=ROOT/'outputs/lp_ml_dataset_v1/plans/lp_ml_dataset_v1_round3_execution_contract_v1.json'
ATTEMPT_ID='LP_ML_ROUND3_TARGETED_ACTIVE_LEARNING_ATTEMPT1_V1'
STAGE=ROOT/'outputs/lp_ml_dataset_v1/staging/lp_ml_dataset_v1_round3_targeted_active_learning_attempt1_v1'
TMP=Path(tempfile.gettempdir())/'lp_ml_dataset_v1_round3_targeted_active_learning_attempt1_runtime'
...
def run_subrun(runtime,r,pol,entered,source_hash,contract_hash):
    ...
    try:
        ...
        entered.append(entry); write_csv(STAGE/'entered_accounting_v1.csv',entered); atomic_json(STAGE/'entered_accounting_v1.json',{'solver_entries':entered,'count':len(entered),'ceiling':128})
...
def main():
    ...
    if len(rows)!=64 or any(r['run_polarizations']!='x,y' for r in rows) or any(r['candidate_id']=='LPML_R1_GLOBAL_SOBOL_054' for r in rows) or len({r['exact_geometry_hash_sha256'] for r in rows})!=64 or len({r['canonical_relative_geometry_hash_sha256'] for r in rows})!=64 or len({r['symmetry_equivalence_geometry_hash_sha256'] for r in rows})!=64: raise SystemExit('HARD_GATE_CONTINUATION_MANIFEST')
    env={'hostname':socket.gethostname(),'cwd':str(ROOT),'branch':subprocess.check_output(['git','branch','--show-current'],cwd=ROOT,text=True).strip(),'head':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'source_contract_sha256':source_hash,'execution_contract_sha256':execution_hash,'protected_report_sha256_before':protected,'solver_authorized_subruns':128,'wavelengths_nm':WLS,'attempt_id':ATTEMPT_ID,'staging_identity':'LP_ML_DATASET_V1_ROUND3_TARGETED_ACTIVE_LEARNING_ATTEMPT1_V1','quarantined_candidate':'LPML_R1_GLOBAL_SOBOL_054','pre_retrain_prediction_status':'ROUND3_PRE_RETRAIN_PROSPECTIVE_EVIDENCE'}
    ...
    for idx,r in enumerate(rows,1):
        results={}; print(f'BEGIN {idx}/64 {r["candidate_id"]}',flush=True)
    ...
    outcome='LP_ML_ROUND3_TARGETED_ACTIVE_LEARNING_PASS_READY_FOR_ASSEMBLY' if entered_n<=128 and not failed_cases and accepted==128 and len(cand)==576 else ('LP_ML_ROUND3_TARGETED_ACTIVE_LEARNING_ISOLATED_FAILURES_DATA_PRESERVED' if entered_n<=128 else 'LP_ML_ROUND3_TARGETED_ACTIVE_LEARNING_HARD_GATE')
    qa={'outcome':outcome,'planned_geometries':64,'planned_subruns':128,'solver_entered':entered_n,'successful_accepted_subruns':accepted,'failed_subruns':len(sub)-accepted,'quarantined_cases':[q['subrun_id'] for q in failed_cases],'complete_recovery_geometries':len(cand)//9,'spectral_rows':len(cand),'duplicate_rows':len(cand)-len({(q['candidate_id'],q['wavelength_nm']) for q in cand}),'model_filled_rows':sum(q.get('model_fill')!='NONE' for q in cand),'protected_report_sha256_before':protected,'protected_report_sha256_after':{str(p):sha_bytes(p.read_bytes()) for p in [ROOT/'reports/lp_ml1a3_git_history_geometry_reconstruction.md',ROOT/'reports/stage11_4a20_legacy_fsp_object_inventory.md']},'no_054_generated':True,'no_accepted_rerun':True,'systemic_failure':systemic,'source_contract_sha256':source_hash,'execution_contract_sha256':execution_hash}
    atomic_json(STAGE/'quality_audit_v1.json',qa); atomic_json(STAGE/'dataset_manifest_v1.json',{'dataset':'LP_ML_DATASET_V1','schema_version':'LP_ML_DATASET_V1','outcome':outcome,'geometry_count':len(cand)//9,'spectral_row_count':len(cand),'solver_entries':entered_n,'physics_label':'FORMAL_WEIGHTED_G0_PROSPECTIVE_ROUND3_TARGETED_ACTIVE_LEARNING','model_filled':False,'no_D9':True,'pre_retrain_predictions_immutable':True}); atomic_json(STAGE/'failure_and_quarantine_v1.json',{'failed_cases':failed_cases,'policy':'isolated entered failures quarantined; no retry; untouched continuation completed'}); atomic_json(STAGE/'final_sentinel_v1.json',{'outcome':outcome,'entered':entered_n,'accepted':accepted,'ceiling':128,'quarantined_count':len(failed_cases)}); atomic_json(STAGE/'checksums_v1.json',{'files':{str(p.relative_to(STAGE)):sha_bytes(p.read_bytes()) for p in STAGE.rglob('*') if p.is_file() and p.name!='checksums_v1.json'}}); print(json.dumps(qa,indent=2,default=str),flush=True)
