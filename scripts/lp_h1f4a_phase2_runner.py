...
REPORT = ROOT / "reports/stage_h1f4a_phase2_grouped_d_transfer_validation"
MANIFEST_PATH = REPORT / "transfer_candidate_manifest.json"
CASE_ROOT = ROOT / "outputs/lp_h1f4a_phase2_grouped_d_transfer_validation/cases"
ACCOUNTING_PATH = REPORT / "h1f4a_phase2_solver_accounting.json"
POLS = ("x", "y")
...
def manifest():
    ...
    if m.get("status") != "FROZEN_READY_FOR_SOLVER" or m.get("candidate_count") != 2:
        raise RuntimeError("HARD_GATE_H1F4A_PHASE2_MANIFEST_NOT_FROZEN")
    ...
    if m.get("wavelength_grid_nm") != GRID or m.get("ml_admitted") is not False or m.get("A_D_nm") != 4.0:
        raise RuntimeError("HARD_GATE_H1F4A_PHASE2_CONTRACT_DRIFT")
    cs = candidates(m)
    if len(cs) != 2 or not {c.get("candidate_uid") for c in cs} == {"H1F4A_K6_L1_B_TRANSFER_PLUS", "H1F4A_K6_L1_B_TRANSFER_MINUS"}:
        raise RuntimeError("HARD_GATE_H1F4A_PHASE2_CHILDREN")
    for c in cs:
        if c.get("grouped_d_mode") != "D_n=D_n_baseline+a_D*cos(2*pi*n/6)+b_D*sin(2*pi*n/6)" or c.get("no_position_modulation") is not True or c.get("helper_J3") is not None:
            raise RuntimeError("HARD_GATE_H1F4A_PHASE2_MODE_DRIFT")
        if c.get("P_supercell_nm") != 2591.446716 or c.get("P_y_nm") != 432.0:
            raise RuntimeError("HARD_GATE_H1F4A_PHASE2_PERIOD_DRIFT")
        if not c.get("geometry_legality", {}).get("pass"):
            raise RuntimeError("HARD_GATE_H1F4A_PHASE2_ILLEGAL_CHILD")
        if c.get("base_candidate_uid") != "K6_L1_B" or c.get("base_candidate_hash") != "ea25ff16c44e2dd00eb9fc6805b6f174a635668f65edad2666f641faf9880a78":
            raise RuntimeError("HARD_GATE_H1F4A_PHASE2_PARENT_HASH_DRIFT")
    if not m.get("rule_artifact_sha256") or m.get("transfer_parent_hash") != "ea25ff16c44e2dd00eb9fc6805b6f174a635668f65edad2666f641faf9880a78":
        raise RuntimeError("HARD_GATE_H1F4A_PHASE2_RULE_OR_PARENT_DRIFT")
    return m
...
def postprocess(m):
    ...
    write_csv(REPORT / "h1f4a_phase2_order_resolved_fullwave.csv", rows)
    jrows = []
    for c in candidates(m):
    ...
    write_csv(REPORT / "h1f4a_phase2_k6_order_jones.csv", jrows)
    accounting = read_json(ACCOUNTING_PATH)
...
def execute():
    ...
    for c in candidates(m):
        for pol in POLS:
            ...
            ledger = read_json(REPORT / "h1f4a_phase2_solver_ledger.json")
            ...
            BASE.write_json(REPORT / "h1f4a_phase2_solver_ledger.json", ledger)
