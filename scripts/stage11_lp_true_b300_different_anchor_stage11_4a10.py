...
RESULT_CSV = REPORT_DIR / "stage11_4a10_true_b300_different_anchor_results.csv"
RANKING_CSV = REPORT_DIR / "stage11_4a10_true_b300_different_anchor_ranking.csv"
SUMMARY_JSON = REPORT_DIR / "stage11_4a10_true_b300_different_anchor_summary.json"
REPORT_MD = REPORT_DIR / "stage11_4a10_true_b300_different_anchor_report.md"
NEXT_MD = REPORT_DIR / "stage11_4a10_true_b300_different_anchor_recommended_next.md"
...
def validate_group() -> None:
    ...
    if not group or int(group["planned_cases"]) != 18:
        raise RuntimeError("A9 G1 must be exactly 18 cases")
...
def source_rows() -> dict[str, dict[str, str]]:
    ...
    for row in data.get("cases", []):
        ...
        if "B300" in cid and cid not in EXCLUDED_SOURCE_IDS and cid not in out:
            out[cid] = dict(row)
    return out
...
def geometries() -> list[dict[str, str]]:
    src = source_rows()
    specs = [
        ("H500DIMER12D_008_B300_y_pair_swap_G80_O-30", "y_pair", "J2-J1", 80.0),
        ("H500DIMER12D_009_B300_y_pair_noswap_G80_O-30", "y_pair", "J1-J2", 80.0),
        ("H500DIMER12D_010_B300_diag_pair_swap_G80_O-30", "diag_pair", "J2-J1", 80.0),
        ("H500DIMER12D_011_B300_diag_pair_noswap_G80_O-30", "diag_pair", "J1-J2", 80.0),
        ("H500DIMER12D_008_B300_y_pair_swap_G80_O-30", "y_pair", "J2-J1", 100.0),
        ("H500DIMER12D_010_B300_diag_pair_swap_G80_O-30", "diag_pair", "J2-J1", 60.0),
    ]
    rows = []
...
def run_matrix() -> list[dict[str, str]]:
    ...
    for idx, base in enumerate(geometries(), start=1):
        for wl in WAVELENGTHS:
            ...
            cid = f"H600TRUEB300G1_{idx:03d}_{base['placement_type']}_{base['swap_order'].replace('-', '')}_G{gap}"
            case_id = f"S11_4A10_TRUE_B300_G1_{idx:03d}_WL{wl}"
            row = dict(base)
            row.update({
                "case_id": case_id,
                ...
                "notes": "Stage11-4A10 true B300 G1 different-anchor only; no G2/G3, coverage, H650, K6.",
            })
            rows.append(row)
    if len(rows) != 18:
        raise RuntimeError(f"Expected 18 A10 rows, got {len(rows)}")
    return rows
