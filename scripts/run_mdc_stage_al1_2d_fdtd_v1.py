"""Authorized 36-case Stage AL-1 runner; no case is retried automatically."""
...
def structures():
 p=pd.read_parquet(SEL/'primary_geometry_matrix.parquet'); b=json.loads((SEL/'staged_budget_recommendation.json').read_text())['stage_AL_1']['primary_geometries'];p=p.set_index('geometry_hash').loc[b].reset_index();out=[]
...
def main(root):
 ...
 state={'task':'APCD_MDC_STAGE_AL1_SIX_GEOMETRY_2D_FDTD_EXECUTION_AND_GATE_V1','created_at':now(),'frozen_selection_root':str(SEL),'cases':cs,'safety_counters':{'FDTD_calls':0,'Lumerical_calls':0,'RCWA_calls':0,'sealed_test_target_reads':0,'TMM_calls':0}};dump(root/'state.json',state);dump(root/'frozen_stage_al1_geometries.json',structures())
