...
PLAN=ROOT/'outputs/lp_ml_dataset_v1/plans/lp_ml_dataset_v1_round1_recovery_389_plan_v1.csv'
CONTRACT=ROOT/'outputs/lp_ml_dataset_v1/plans/lp_ml_dataset_v1_contract_v1.json'
ECONTRACT=ROOT/'outputs/lp_ml_dataset_v1/plans/lp_ml_dataset_v1_round1_recovery_execution_contract_v1.json'
ATTEMPT_ID='LP_ML_ROUND1_RECOVERY_ATTEMPT1_V1'
STAGE=ROOT/'outputs/lp_ml_dataset_v1/staging/lp_ml_dataset_v1_round1_recovery_attempt1_v1'
TMP=Path(tempfile.gettempdir())/'lp_ml_dataset_v1_round1_recovery_attempt1_runtime'
...
def run_subrun(runtime,r,pol,entered,source_hash,contract_hash):
    ...
    try:
        ...
        entered.append(entry); write_csv(STAGE/'entered_accounting_v1.csv',entered); atomic_json(STAGE/'entered_accounting_v1.json',{'solver_entries':entered,'count':len(entered),'ceiling':389})
...
def main():
    ...
    if len(rows)!=195 or rows[0]['candidate_id']!='LPML_R1_GLOBAL_SOBOL_054' or rows[0]['run_polarizations']!='y': raise SystemExit('HARD_GATE_RECOVERY_MANIFEST')
    env={'hostname':socket.gethostname(),'cwd':str(ROOT),'branch':subprocess.check_output(['git','branch','--show-current'],cwd=ROOT,text=True).strip(),'head':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'source_contract_sha256':source_hash,'execution_contract_sha256':execution_hash,'protected_report_sha256_before':protected,'solver_authorized_subruns':389,'wavelengths_nm':WLS,'attempt_id':ATTEMPT_ID,'staging_identity':'LP_ML_DATASET_V1_ROUND1_RECOVERY_ATTEMPT1_V1'}
    ...
    if args.preflight_only:
        r=rows[0]; f=lumapi.FDTD(hide=getattr(runtime,'hide_gui',True)); fp=TMP/'preflight_sampling_gate.fsp'; setup=build(f,r,'y'); f.save(str(fp)); pre_hash=sha_bytes(fp.read_bytes()); f.close(); f=lumapi.FDTD(hide=getattr(runtime,'hide_gui',True)); f.load(str(fp)); gate=build_gate(f,r,'y'); f.close(); fp.unlink(missing_ok=True); env.update({'preflight_setup':setup,'preflight_reload_gate':gate,'solver_entered':False,'preflight_polarization':'y'}); atomic_json(ROOT/'outputs/lp_ml_dataset_v1/analysis/lp_ml_dataset_v1_round1_recovery_preflight_sampling_gate_v1.json',env); print(json.dumps(env,indent=2,default=str)); return
    atomic_json(STAGE/'preflight_environment_v1.json',env); entered=[]; sub=[]; cand=[]; failed_rec=None
    for idx,r in enumerate(rows,1):
        pols=[p for p in r['run_polarizations'].split(',') if p]
        print(f'BEGIN {idx}/195 {r["candidate_id"]} {pols}',flush=True); results={}
        for pol in pols:
            q=run_subrun(proxy,r,pol,entered,source_hash,contract_hash); sub.append(q); results[pol]=q
            if q['status']!='ACCEPTED':
                failed_rec=q; atomic_json(STAGE/'failure_evidence_v1.json',{'outcome':'LP_ML_ROUND1_RECOVERY_HARD_GATE','failed_subrun':q,'entered_count':len(entered),'protected_before':protected}); break
        if failed_rec is not None: break
        if pols==['x','y']:
            cr=candidate_rows(r,results['x'],results['y']); cp=STAGE/'candidates'/r['candidate_id']; cp.mkdir(parents=True,exist_ok=True); atomic_json(cp/'candidate_checkpoint.json',{'candidate_id':r['candidate_id'],'geometry':dict(r),'rows':cr,'x_checkpoint':results['x']['checkpoint_path'],'y_checkpoint':results['y']['checkpoint_path'],'status':'ACCEPTED'}); cand.extend(cr); print(f'ACCEPT {idx}/195 {r["candidate_id"]}',flush=True)
        else: print(f'ACCEPT_MISSING_Y {r["candidate_id"]}',flush=True)
    write_csv(STAGE/'subrun_records_v1.csv',[{k:v for k,v in q.items() if k!='rows'} for q in sub]); write_csv(STAGE/'candidate_wavelength_jones_v1.csv',cand); write_csv(STAGE/'geometry_records_v1.csv',[dict(r) for r in rows if r['candidate_id'] in {q['candidate_id'] for q in cand}])
    accepted=sum(q['status']=='ACCEPTED' for q in sub); entered_n=len(entered); outcome='LP_ML_ROUND1_RECOVERY_PASS_READY_FOR_ASSEMBLY' if failed_rec is None and accepted==389 and len(cand)==1746 else 'LP_ML_ROUND1_RECOVERY_HARD_GATE'
    qa={'outcome':outcome,'planned_geometries':195,'planned_subruns':389,'solver_entered':entered_n,'successful_accepted_subruns':accepted,'failed_subruns':len(sub)-accepted,'complete_recovery_geometries':len(cand)//9,'spectral_rows':len(cand),'duplicate_rows':len(cand)-len({(q['candidate_id'],q['wavelength_nm']) for q in cand}),'model_filled_rows':sum(q.get('model_fill')!='NONE' for q in cand),'protected_report_sha256_before':protected,'protected_report_sha256_after':{str(p):sha_bytes(p.read_bytes()) for p in [ROOT/'reports/lp_ml1a3_git_history_geometry_reconstruction.md',ROOT/'reports/stage11_4a20_legacy_fsp_object_inventory.md']},'no_d9_generated':True,'old_054_x_rerun':False,'old_failed_attempt_overwritten':False,'source_contract_sha256':source_hash,'execution_contract_sha256':execution_hash}
    atomic_json(STAGE/'quality_audit_v1.json',qa); atomic_json(STAGE/'dataset_manifest_v1.json',{'dataset':'LP_ML_DATASET_V1','schema_version':'LP_ML_DATASET_V1','outcome':outcome,'geometry_count':len(cand)//9,'spectral_row_count':len(cand),'solver_entries':entered_n,'physics_label':'FORMAL_WEIGHTED_G0_PROSPECTIVE_RECOVERY','model_filled':False,'no_D9':True}); atomic_json(STAGE/'final_sentinel_v1.json',{'outcome':outcome,'entered':entered_n,'accepted':accepted,'ceiling':389,'first_candidate_gate_passed':failed_rec is None}); atomic_json(STAGE/'checksums_v1.json',{'files':{str(p.relative_to(STAGE)):sha_bytes(p.read_bytes()) for p in STAGE.rglob('*') if p.is_file() and p.name!='checksums_v1.json'}}); print(json.dumps(qa,indent=2,default=str),flush=True)
