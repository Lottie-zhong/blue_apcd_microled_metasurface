"""Authorized 24-case Pilot4 joint wavelength-angle profile runner.

Uses the frozen six-case matrix, the validated Native-M1 builder primitives and
...
CASE_MATRIX=ROOT/'contracts/mdc_hf_surrogate_v2/fixed_v2_pilot4_case_matrix.csv'
CANDIDATES=ROOT/'contracts/mdc_hf_surrogate_v2/fixed_v2_pilot4_candidate_manifest.json'
...
RUNTIME_ROOT=ROOT/'runtime/mdc_hf_surrogate_v2_pilot4_joint_profile_v1'
...
def read_matrix():
    ...
    if len(rows)!=24: raise RuntimeError('pilot4_case_count_drift')
    if len({r['geometry_hash'] for r in rows})!=4 or len({r['case_hash'] for r in rows})!=24: raise RuntimeError('pilot4_hash_count_drift')
    if any(r['dipole_orientation'] not in ('x','z') for r in rows): raise RuntimeError('pilot4_orientation_drift')
    if any(r['source_position'] not in ('top','centroid','bottom') for r in rows): raise RuntimeError('pilot4_source_position_drift')
    expected={'top':'-171.5','centroid':'-276.0','bottom':'-380.5'}
    if any(r['source_position_nm']!=expected[r['source_position']] for r in rows): raise RuntimeError('pilot4_source_nm_drift')
    if any(r['dipole_orientation']=='y' for r in rows): raise RuntimeError('pilot4_y_dipole_forbidden')
    counts={g:sum(r['geometry_hash']==g for r in rows) for g in {r['geometry_hash'] for r in rows}}
    if set(counts.values())!={6}: raise RuntimeError('pilot4_cases_per_geometry_drift')
    return rows

def candidate_map():
    c=load(CANDIDATES)
    if c.get('candidate_count')!=4 or c.get('case_count')!=24: raise RuntimeError('pilot4_candidate_manifest_drift')
...
def extract_case(case,run_root,state,smoke=False):
    ...
    attempt_id=f"{case['case_hash'][:12]}_attempt_1"; pre=case_dir/(attempt_id+'__pre.fsp'); post=case_dir/(attempt_id+'__post.fsp'); npz=case_dir/(attempt_id+'__raw.npz')
    ...
    try:
        ...
        with (run_root/'pilot4_case_attempt_ledger.jsonl').open('a',encoding='utf-8') as lf: lf.write(json.dumps(ledger,sort_keys=True)+'\n')
        ...
        state['cases'][case['case_hash']].update(case_result); state['safety_counters']['solver_calls']+=1; state['safety_counters']['fdtd_lumerical_calls']+=1; dump(run_root/'state.json',state); dump(case_dir/'case_result.json',case_result)
        return case_result
    except Exception:
...
def initialize(run_root):
    ...
    if len(cand)!=4: raise RuntimeError('candidate_count_drift')
    auth={'solver_authorized':True,'authorized_tier':'PILOT4','authorized_geometry_count':4,'authorized_case_count':24,'authorization_source':'EXPLICIT_USER_APPROVAL','authorization_date':'2026-08-03','DOE96_authorized':False,'HF15_formal_reads_authorized':False,'sealed_test_authorized':False}
    dump(run_root/'pilot4_solver_authorization.json',auth)
...
def smoke_audit(run_root,state,case_result):
    ...
    dump(run_root/'pilot4_upgrade_smoke_audit.json',a); return a

def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--output-root',required=True); ap.add_argument('--init',action='store_true'); ap.add_argument('--smoke',action='store_true'); ap.add_argument('--continue-run',action='store_true'); args=ap.parse_args(); run_root=Path(args.output_root)
    ...
    if args.continue_run:
        smoke=load(run_root/'pilot4_upgrade_smoke_audit.json')
        ...
        for case in list(state['cases'].values()):
            if case['status']=='PENDING': extract_case(case,run_root,state)
            state=load(run_root/'state.json')
        dump(run_root/'pilot4_solver_run_manifest.json',{'run_id':run_root.name,'authorized_geometry_count':4,'authorized_unique_physical_cases':24,'completed_unique_physical_cases':sum(c.get('solver_status')=='COMPLETE' for c in state['cases'].values()),'accepted_cases':sum(bool(c.get('accepted')) for c in state['cases'].values()),'rejected_unresolved_cases':sum(not bool(c.get('accepted')) for c in state['cases'].values()),'total_solver_calls':state['safety_counters']['solver_calls'],'recovery_solver_calls':state['safety_counters']['recovery_solver_calls'],'DOE96_solver_calls':0,'status':'PILOT4_SOLVER_RUN_COMPLETE' if all(c.get('accepted') for c in state['cases'].values()) else 'HARD_GATE_PILOT4_CASE_UNRECOVERABLE','safety_counters':state['safety_counters']})
        print(json.dumps(load(run_root/'pilot4_solver_run_manifest.json'),sort_keys=True))
