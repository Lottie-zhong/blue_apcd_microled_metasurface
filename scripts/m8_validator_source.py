...
for audit_name in ('authority_full_audit.json','m8_requirement_audit.json','residual_reconstruction_audit.json','schema_and_leakage_audit.json','selection_time_preservation_audit.json','regression_test_audit.json','ranking_seed_stability.json'):
    audit = j(audit_name)
    ...
    if audit_name == 'ranking_seed_stability.json' and (audit.get('status') != 'PASS' or audit.get('seed_count') != 3): errors.append('ranking_seed_stability')
