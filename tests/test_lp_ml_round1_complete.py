...
def test_from_scratch_models_and_contracts():
    m=json.loads((A/'lp_ml_round1_full_residual_mlp_5seed_v1.json').read_text()); assert m['from_scratch'] and not m['warm_start']; assert len(m['seeds'])==5; assert m['cuda_available']; assert 'SmoothL1' in m['loss']
