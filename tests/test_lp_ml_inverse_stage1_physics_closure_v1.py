...
class Stage1ClosureTest(unittest.TestCase):
 def test_counts_and_tuple_gate(self):
  ...
  q=json.loads((A/'lp_ml_inverse_stage1_physics_closure_decision_v1.json').read_text());self.assertEqual(q['complete_counts']['B2'],5);self.assertEqual(q['raw_tuple_combinations'],38880);self.assertEqual(q['solver_calls'],0)
 ...
 def test_no_execution_proposal(self):
  p=json.loads((A/'lp_ml_inverse_stage1_broadband_confirmation_proposal_v1.json').read_text());self.assertTrue(p['no_execution_in_this_task']);self.assertEqual(p['wavelengths_nm'][0],450.0);self.assertEqual(p['wavelengths_nm'][-1],454.0)
