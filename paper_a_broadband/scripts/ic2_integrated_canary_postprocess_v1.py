...
import sys
for _api_path in (
    r"N:\Program Files\ANSYS Inc\v251\Lumerical\api\python",
...
def main():
    ...
    if provenance.get("case_id") != CASE_ID or provenance.get("status") != "RETURNED":
        raise RuntimeError("IC2_PROVENANCE_NOT_RETURNED")
    readback = json.loads(args.readback.read_text(encoding="utf-8"))
    physics_semantic = readback.get("physics_semantic", {})
    source = physics_semantic.get("source", {})
    source_position_ok = all(
        abs(float(source.get(key, float("nan"))) - expected) <= 1e-12
        for key, expected in (("x", 0.0), ("y", 0.0), ("z", -1.715e-7))
    )
    source_y_ok = abs(float(source.get("theta", float("nan"))) - 90.0) <= 1e-9 and abs(float(source.get("phi", float("nan"))) - 90.0) <= 1e-9
    object_inventory = set(readback.get("object_inventory", []))
    required_objects = {name for name, _ in FACES} | {"ic1_top_near_to_far", "ic1_v2_time_probe"}
    semantic_audit = {
        "case_id": readback.get("physics_semantic", {}).get("case_id"),
        "readback_status": readback.get("status"),
        "native_materials": physics_semantic.get("native_material_ids"),
        "source_position_m": {"x": source.get("x"), "y": source.get("y"), "z": source.get("z")},
        "source_orientation": {"theta_deg": source.get("theta"), "phi_deg": source.get("phi"), "expected": "y-oriented; theta=90 deg, phi=90 deg"},
        "source_position_ok": source_position_ok,
        "source_y_orientation_ok": source_y_ok,
        "physics_semantic_fingerprint": readback.get("physics_semantic_fingerprint"),
        "integrated_instrumentation_fingerprint": readback.get("integrated_instrumentation_fingerprint"),
        "expected_shared_physics_semantic_fingerprint": "43f8469cd278ffa6d56876391447a3c3bf902be03b17b362e8d844dfe43eeb0a",
        "expected_shared_integrated_instrumentation_fingerprint": "ff0f40c8595b71d7bd1d5d16db898338b9dbfbb1f92fbe5978f6ab1beb163b86",
        "required_instrumentation_objects_present": required_objects.issubset(object_inventory),
        "object_name_note": "IC2 preserves IC1 instrumentation object names to keep the validated measurement contract identical; source orientation is the intended x-to-y delta.",
        "all_geometry_mdc_i03_domain_monitor_settings_shared": readback.get("physics_semantic_fingerprint") == "43f8469cd278ffa6d56876391447a3c3bf902be03b17b362e8d844dfe43eeb0a" and readback.get("integrated_instrumentation_fingerprint") == "ff0f40c8595b71d7bd1d5d16db898338b9dbfbb1f92fbe5978f6ab1beb163b86",
    }
    if readback.get("status") != "PASS" or not source_position_ok or not source_y_ok or not semantic_audit["required_instrumentation_objects_present"] or not semantic_audit["all_geometry_mdc_i03_domain_monitor_settings_shared"]:
        raise RuntimeError("IC2_TERMINAL_SEMANTIC_AUDIT_FAILED")
    post_sha = sha256(args.post_fsp)
    if post_sha != provenance.get("post_fsp_sha256"):
        raise RuntimeError("IC2_POST_FSP_SHA_MISMATCH")
    ...
    try:
        ...
        if len(wavelength_nm) != 101 or not np.all(np.isfinite(wavelength_nm)):
            raise RuntimeError("IC2_SPECTRAL_GRID_INVALID")
        ...
        for name, normal in FACES:
            ...
            if len(transmission) != len(wavelength_nm) or len(power_result) != len(wavelength_nm):
                raise RuntimeError(f"IC2_FACE_GRID_INVALID:{name}")
        ...
        if e_plane.ndim != 5 or e_plane.shape[-1] < 2 or e_plane.shape[3] != len(wavelength_nm):
            raise RuntimeError("IC2_NEAR_TO_FAR_E_GRID_INVALID")
        ...
        for i, wl in enumerate(wavelength_nm):
            ...
            if vector.size < 2 or not np.all(np.isfinite(vector)) or not np.all(np.isfinite(intensity)):
                raise RuntimeError(f"IC2_FARFIELD_NONFINITE:{i}")
        ...
        write_json(args.runtime_dir / "ic2_convergence_evidence_v2.json", convergence)
        write_csv(args.output_dir / "ic2_closed_flux.csv", closed_rows, list(closed_rows[0]))
        write_csv(args.output_dir / "ic2_stokes.csv", stokes_rows, list(stokes_rows[0]))
        write_csv(args.output_dir / "ic2_farfield_metrics.csv", farfield_rows, list(farfield_rows[0]))
        write_csv(args.output_dir / "ic2_face_flux_long.csv", face_rows, list(face_rows[0]))
        ...
        if not source_ok:
            ...
            classification = "VALID_FOR_IC2_INTEGRATED_TRUTH"
            root = None
        validity = {
            "schema": "PAPER_A_FDTD_PHYSICS_VALIDITY_GATE_RESULT_V2_IC2_INTEGRATED_ADAPTER",
            "case_id": CASE_ID, "status": classification, "root_cause": root,
            "authority": {"v2": "paper_a_broadband/authority/paper_a_fdtd_physics_validity_gate_v2_instrumented.json", "adapter": "shared IC1/IC2 integrated V2 contract with IC2 y-orientation audit", "no_threshold_invention": True},
            "gates": {
            ...
            "solver_accounting": {"authorized": 1, "entered": 1, "returned": 1, "accepted": 1 if classification == "VALID_FOR_IC2_INTEGRATED_TRUTH" else 0, "additional_replay": 0, "postprocess_solver_run_called": False, "postprocess_solver_entered": 0},
            "W_emit": {"status": "UNRESOLVED_FOR_PRODUCTION_CLOSURE", "used": False},
            "forbidden_cases_started": [], "timestamp_utc": now(),
        }
        write_json(args.output_dir / "ic2_integrated_validity_gate_v2.json", validity)
        summary = {
            "schema": "PAPER_A_IC2_INTEGRATED_POSTPROCESS_SUMMARY_V1", "case_id": CASE_ID, "classification": classification, "root_cause": root,
            ...
            "architecture_verdict": "PAPER_A_IC2_FINITE_INTEGRATED_PASS" if classification == "VALID_FOR_IC2_INTEGRATED_TRUTH" else "PAPER_A_IC2_FINITE_INTEGRATED_NOT_VALIDATED",
            "W_emit": "UNRESOLVED_FOR_PRODUCTION_CLOSURE", "raw_solver_data_modified": False, "timestamp_utc": now(),
        }
        summary["terminal_provenance_audit"] = semantic_audit
        write_json(args.output_dir / "ic2_integrated_canary_summary.json", summary)
        write_json(args.output_dir / "ic2_terminal_postprocess.json", {
            "schema": "PAPER_A_IC2_TERMINAL_POSTPROCESS_AUDIT_V1",
            "case_id": CASE_ID,
            "classification": classification,
            "provenance": {
                "attempt_id": provenance.get("attempt_id"),
                "status": provenance.get("status"),
                "solver_entered": provenance.get("solver_entered"),
                "solver_run_called": provenance.get("solver_run_called"),
                "mpi_processes": provenance.get("mpi_processes"),
                "threads_per_process": provenance.get("threads_per_process"),
                "no_auto_replay": provenance.get("no_auto_replay"),
                "pre_fsp_sha256": provenance.get("pre_fsp_sha256"),
                "post_fsp_sha256": post_sha,
                "post_fsp_present": bool(args.post_fsp.exists()),
            },
            "semantic_audit": semantic_audit,
            "postprocess_solver_run_called": False,
            "postprocess_solver_entered": 0,
            "raw_solver_data_modified": False,
